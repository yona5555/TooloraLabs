...
import RetirementStartEarlyChart from "./RetirementStartEarlyChart";

type ExampleRow = { scenario: string; projectedBalance: string; totalGrowth: string };
type VariableItem = { name: string; description: string };
type ApplicationItem = { title: string; description: string };

export default async function RetirementEducation() {
  const t = await getTranslations("tools.retirement-calculator.education");
          ...
          <table className="w-full min-w-[480px] border-collapse text-sm">
            <thead>
              <tr className="border-b border-current/30 text-start">
                ...
                <th className="px-3 py-2 text-start font-semibold">{t("examples.columnProjectedBalance")}</th>
                <th className="px-3 py-2 text-start font-semibold">{t("examples.columnTotalGrowth")}</th>
              </tr>
            </thead>
            <tbody>
              {exampleRows.map((row) => (
                <tr key={row.scenario} className="border-b border-current/10">
                  ...
                  <td className="px-3 py-2.5 font-mono font-semibold">{row.projectedBalance}</td>
                  <td className="px-3 py-2.5 font-mono font-semibold">{row.totalGrowth}</td>
                </tr>
              ))}
            </tbody>
          </table>
