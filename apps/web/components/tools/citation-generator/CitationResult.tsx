...
export default function CitationResult({ result, draft }: Props) {
  ...
  const completenessColor =
    completenessBand === "complete"
      ...
      : completenessBand === "mostlyComplete"
        ? "fill-blue-600 dark:fill-blue-400"
        : "fill-amber-600 dark:fill-amber-400";

  const completenessGauge = (
    <div className="flex justify-center p-4 lg:p-6">
      <RatioGauge
        ...
        domainMax={100}
        zones={[
          { key: "incomplete", from: 0, to: 60, colorClass: "stroke-amber-500 dark:stroke-amber-400" },
          { key: "mostlyComplete", from: 60, to: 90, colorClass: "stroke-blue-500 dark:stroke-blue-400" },
          { key: "complete", from: 90, to: 100, colorClass: "stroke-emerald-500 dark:stroke-emerald-400" },
        ]}
        valueLabel={`${completeness.filled}/${completeness.total}`}
        caption={t(`completeness.${completenessBand}`)}
        captionColorClass={completenessColor}
        ticks={[0, 60, 90, 100]}
      />
    </div>
  );
