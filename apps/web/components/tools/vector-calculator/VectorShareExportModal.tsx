...
export default function VectorShareExportModal({ result, ax, ay, az, bx, by, bz, digitStyle }: Props) {
  const t = useTranslations("tools.vector-calculator");
  const tResult = useTranslations("tools.vector-calculator.result");
  ...
  const na = tResult("notApplicable");
  const shareUrl = typeof window !== "undefined" ? window.location.href : "";
  const shareText = `${t("title")}: A·B = ${fmt(result.dotProduct)}`;

  const inputRows = [
    { label: "A", value: `(${fmt(ax)}, ${fmt(ay)}, ${fmt(az)})` },
    { label: "B", value: `(${fmt(bx)}, ${fmt(by)}, ${fmt(bz)})` },
  ];

  const resultRows = [
    { label: tResult("magnitudeA"), value: fmt(result.magnitudeA) },
    { label: tResult("magnitudeB"), value: fmt(result.magnitudeB) },
    { label: tResult("dotProduct"), value: fmt(result.dotProduct) },
    { label: tResult("sum"), value: `(${fmt(result.sumX)}, ${fmt(result.sumY)}, ${fmt(result.sumZ)})` },
    { label: tResult("difference"), value: `(${fmt(result.differenceX)}, ${fmt(result.differenceY)}, ${fmt(result.differenceZ)})` },
    { label: tResult("crossProduct"), value: `(${fmt(result.crossX)}, ${fmt(result.crossY)}, ${fmt(result.crossZ)})` },
    { label: tResult("angleBetween"), value: result.angleBetweenDegrees === null ? na : `${fmt(result.angleBetweenDegrees)}°` },
    { label: tResult("unitVectorA"), value: result.unitAX === null ? na : `(${fmt(result.unitAX)}, ${fmt(result.unitAY!)}, ${fmt(result.unitAZ!)})` },
    { label: tResult("projectionAOntoB"), value: result.projectionX === null ? na : `(${fmt(result.projectionX)}, ${fmt(result.projectionY!)}, ${fmt(result.projectionZ!)})` },
  ];
  ...
  async function handleDownloadPdf() {
    ...
    try {
      await generateToolPdf({
        ...
        filename: "vector-calculator.pdf",
      });
      setPdfState("done");
