      ...
      <div dir="ltr" className="overflow-x-auto">
        <svg viewBox={`0 0 ${WIDTH} ${height}`} role="img" aria-label={caption} className="h-auto w-full text-current" style={{ minWidth: 280 }}>
          {rows.map((row, i) => {
            ...
            const width = Math.max((row.value / maxValue) * barMaxWidth, 2);
            return (
              <g key={i}>
                <text x={LABEL_WIDTH - 8} y={y + ROW_HEIGHT / 2 + 4} textAnchor="end" fontSize={10} fill="currentColor" className="truncate">
                  {row.label.length > 14 ? `${row.label.slice(0, 13)}…` : row.label}
                </text>
                <rect x={LABEL_WIDTH} y={y} width={width} height={ROW_HEIGHT} rx={4} className="fill-blue-600 dark:fill-blue-400" />
