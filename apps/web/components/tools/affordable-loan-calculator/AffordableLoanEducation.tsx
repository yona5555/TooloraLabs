...
export default async function AffordableLoanEducation() {
  const t = await getTranslations("tools.affordable-loan-calculator.education");
          ...
          <table className="w-full min-w-[480px] border-collapse text-sm">
            <thead>
              <tr className="border-b border-current/30 text-start">
                ...
                <th className="px-3 py-2 text-start font-semibold">{t("examples.columnMaxLoanAmount")}</th>
                <th className="px-3 py-2 text-start font-semibold">{t("examples.columnTotalInterest")}</th>
              </tr>
            </thead>
            <tbody>
              {exampleRows.map((row) => (
                <tr key={row.scenario} className="border-b border-current/10">
                  ...
                  <td className="px-3 py-2.5 font-mono font-semibold">{row.maxLoanAmount}</td>
                  <td className="px-3 py-2.5 font-mono font-semibold">{row.totalInterest}</td>
                </tr>
              ))}
            </tbody>
          </table>
