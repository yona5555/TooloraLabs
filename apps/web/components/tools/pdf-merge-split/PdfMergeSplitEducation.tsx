...
export default async function PdfMergeSplitEducation() {
  const t = await getTranslations("tools.pdf-merge-split.education");
            ...
            <tbody>
              {exampleRows.map((row) => (
                <tr key={row.scenario} className="border-b border-current/10">
                  ...
                  <td className="px-3 py-2.5 font-mono">{row.result}</td>
                </tr>
              ))}
            </tbody>
          </table>
