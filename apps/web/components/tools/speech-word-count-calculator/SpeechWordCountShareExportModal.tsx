...
type SpeechWordCountShareExportModalProps = {
  inputRows: Row[];
  resultRows: Row[];
  heroLabel: string;
  heroValue: string;
...
export default function SpeechWordCountShareExportModal({ inputRows, resultRows, heroLabel, heroValue, sentence }: SpeechWordCountShareExportModalProps) {
  const t = useTranslations("tools.speech-word-count-calculator");
  ...
  async function handleDownloadPdf() {
    ...
    try {
      await generateToolPdf({
        ...
        filename: "speech-word-count-calculator.pdf",
      });
      setPdfState("done");
  ...
  return (
    <>
      ...
      <div data-print-area className="hidden bg-white p-8 text-black print:block">
        ...
        <h1 className="mb-4 text-2xl font-bold text-zinc-900">{t("title")}</h1>
