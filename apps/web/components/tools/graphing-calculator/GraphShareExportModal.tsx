...
export default function GraphShareExportModal({ expression, xMin, xMax, yMin, yMax, tableRows }: Props) {
  const t = useTranslations("tools.graphing-calculator");
  const tForm = useTranslations("tools.graphing-calculator.form");
  const tResult = useTranslations("tools.graphing-calculator.result");
  ...
  const shareText = `${t("title")}: y = ${expression}`;

  const inputRows = [
    { label: tForm("expressionLabel"), value: expression },
    { label: tForm("xMinLabel"), value: fmt(xMin) },
    { label: tForm("xMaxLabel"), value: fmt(xMax) },
  ];

  const resultRows = [{ label: tResult("yRangeLabel"), value: `[${fmt(yMin)}, ${fmt(yMax)}]` }];

  const valuesTable = {
    title: tResult("tableOfValues"),
    columns: ["x", "y"],
    rows: tableRows.map((row) => [fmt(row.x), row.y === null ? tResult("undefined") : fmt(row.y)]),
  };
  ...
  async function handleDownloadPdf() {
    ...
    try {
      await generateToolPdf({
        ...
        filename: "graphing-calculator.pdf",
      });
      setPdfState("done");
  ...
  return (
    <>
      ...
      <div data-print-area className="hidden bg-white p-8 text-black print:block">
        ...
        <h2 className="mb-2 mt-6 text-sm font-bold text-blue-600">{valuesTable.title}</h2>
        <table className="w-full border-collapse text-sm">
          <thead>
            <tr>
              {valuesTable.columns.map((col) => (
