...
export default function MathSolverShareExportModal({ result, draft }: Props) {
  const t = useTranslations("tools.step-by-step-math-solver");
  const tForm = useTranslations("tools.step-by-step-math-solver.form");
  ...
  const finalAnswer = result.error ? "" : result.result;
  const shareText = `${t("title")}: ${finalAnswer}`;

  const inputRows: Array<{ label: string; value: string }> =
    draft.mode === "linear-equation"
      ? [
          { label: "a", value: draft.linearA || "—" },
          { label: "b", value: draft.linearB || "—" },
          { label: "c", value: draft.linearC || "—" },
          { label: "d", value: draft.linearD || "—" },
        ]
      : draft.mode === "quadratic-equation"
        ? [
            { label: "a", value: draft.quadA || "—" },
            { label: "b", value: draft.quadB || "—" },
            { label: "c", value: draft.quadC || "—" },
          ]
        : draft.mode === "fraction-operation"
          ? [
              { label: tForm("numeratorLabel"), value: draft.fracA || "—" },
              { label: tForm("denominatorLabel"), value: draft.fracB || "—" },
              { label: tForm("operationLabel"), value: tForm(`fracOp.${draft.fracOp}`) },
              { label: tForm("numeratorLabel"), value: draft.fracC || "—" },
              { label: tForm("denominatorLabel"), value: draft.fracD || "—" },
            ]
          : draft.polynomialTerms.map((term, i) => ({
              label: `${tForm("coefficientLabel")} / ${tForm("powerLabel")} #${i + 1}`,
              value: `${term.coefficient || "—"} / ${term.power || "—"}`,
            }));

  const resultRows = result.error ? [] : [{ label: t("result.heading"), value: finalAnswer }];

  const stepsTable = result.error
    ? { title: "", columns: [], rows: [] }
    : {
        title: t("shareExport.stepsTableTitle"),
        columns: [t("shareExport.stepColumn"), t("shareExport.descriptionColumn")],
        rows: result.steps.map((step, i) => [String(i + 1), step]),
      };
  ...
  async function handleDownloadPdf() {
    ...
    try {
      await generateToolPdf({
        locale,
        toolName: `${t("title")} — ${tForm(`mode.${draft.mode}`)}`,
        inputs: inputRows,
        results: resultRows,
        table: stepsTable,
        ...
        filename: "step-by-step-math-solver.pdf",
      });
      setPdfState("done");
    ...
    <>
      ...
      <div data-print-area className="hidden bg-white p-8 text-black print:block">
        ...
        <h1 className="mb-4 text-xl font-bold text-zinc-900">
          {t("title")} — {tForm(`mode.${draft.mode}`)}
        </h1>

        <h2 className="mb-2 mt-6 text-sm font-bold text-blue-600">{tForm("inputTitle")}</h2>
        ...
        <table className="w-full border-collapse text-sm">
          <tbody>
            {resultRows.map((row) => (
              <tr key={row.label}>
                <td className="border border-zinc-200 bg-blue-50 px-3 py-1.5 font-semibold text-blue-900">{row.label}</td>
                <td className="border border-zinc-200 px-3 py-1.5 font-semibold">{row.value}</td>
              </tr>
            ))}
          </tbody>
        </table>

        {stepsTable.rows.length > 0 && (
          <>
            <h2 className="mb-2 mt-6 text-sm font-bold text-blue-600">{stepsTable.title}</h2>
            <table className="w-full border-collapse text-sm">
              <thead>
                <tr>
                  {stepsTable.columns.map((col) => (
                    <th key={col} className="border border-zinc-200 bg-blue-50 px-3 py-1.5 text-start font-semibold text-blue-900">
                      {col}
                    </th>
                  ))}
                </tr>
              ...
              <tbody>
                {stepsTable.rows.map((row, i) => (
                  <tr key={i}>
                    {row.map((cell, j) => (
                      <td key={j} className="border border-zinc-200 px-3 py-1.5">
                        {cell}
                      </td>
                    ))}
                  </tr>
