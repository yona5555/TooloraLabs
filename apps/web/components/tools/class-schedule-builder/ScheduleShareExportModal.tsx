...
type ScheduleShareExportModalProps = {
...
export default function ScheduleShareExportModal({ operationLabel, inputRows, resultRows, heroLabel, heroValue, sentence }: ScheduleShareExportModalProps) {
  const t = useTranslations("tools.class-schedule-builder");
  ...
  async function handleDownloadPdf() {
    ...
    try {
      await generateToolPdf({
        locale,
        toolName: operationLabel ? `${t("title")} — ${operationLabel}` : t("title"),
        inputs: inputRows,
        results: resultRows,
        sentence,
        ...
        filename: "class-schedule-builder.pdf",
      });
      setPdfState("done");
  ...
  return (
    <>
      ...
      <div data-print-area className="hidden bg-white p-8 text-black print:block">
        ...
        <h1 className="mb-4 text-2xl font-bold text-zinc-900">{operationLabel ? `${t("title")} — ${operationLabel}` : t("title")}</h1>

        {inputRows.length > 0 && (
          <>
            <h2 className="mb-2 mt-6 text-base font-bold text-blue-600">{t("shareExport.inputSectionTitle")}</h2>
        ...
        <table className="w-full border-collapse text-base">
          <tbody>
            <tr>
              ...
              <td dir="ltr" className="border border-zinc-200 px-3 py-2 font-semibold">{heroValue}</td>
            </tr>
            {resultRows
