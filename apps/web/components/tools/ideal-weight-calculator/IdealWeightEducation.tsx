...
export default async function IdealWeightEducation() {
  const t = await getTranslations("tools.ideal-weight-calculator.education");
          ...
          <table className="w-full min-w-[480px] border-collapse text-sm">
            <thead>
              <tr className="border-b border-current/30 text-start">
                ...
                <th className="px-3 py-2 text-start font-semibold">{t("examples.columnAverage")}</th>
                <th className="px-3 py-2 text-start font-semibold">{t("examples.columnRange")}</th>
              </tr>
            </thead>
            <tbody>
              {exampleRows.map((row) => (
                <tr key={row.scenario} className="border-b border-current/10">
                  ...
                  <td className="px-3 py-2.5 font-mono font-semibold">{row.average}</td>
                  <td className="px-3 py-2.5 font-mono font-semibold">{row.range}</td>
                </tr>
              ))}
            </tbody>
          </table>
