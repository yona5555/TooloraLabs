...
type WeatherShareExportModalProps = {
  inputRows: Row[];
  resultRows: Row[];
  heroLabel: string;
  heroValue: string;
...
export default function WeatherShareExportModal({ inputRows, resultRows, heroLabel, heroValue, sentence, gauge }: WeatherShareExportModalProps) {
  const t = useTranslations("tools.weather-forecast");
  ...
  return (
    <>
      <button
        ...
        className="flex items-center gap-1.5 rounded-lg border border-white/30 px-3 py-1.5 text-xs font-medium text-white/90 transition hover:bg-white/10"
      >
        <Share2 size={14} />
      ...
      <div data-print-area className="hidden bg-white p-8 text-black print:block">
        ...
        <h2 className="mb-2 mt-6 text-sm font-bold text-blue-600">{t("aboveFold.cityPanelTitle")}</h2>
        ...
        <h2 className="mb-2 mt-6 text-sm font-bold text-blue-600">{t("forecast.title")}</h2>
        <table className="w-full border-collapse text-sm">
          <tbody>
            <tr>
