...
type ExampleRow = { scenario: string; probability: string };
type UseCaseItem = { title: string; description: string };

export default async function DiceEducation() {
  const t = await getTranslations("tools.dice-roller.education");
          ...
          <table className="w-full min-w-[420px] border-collapse text-sm">
            <thead>
              <tr className="border-b border-current/30 text-start">
                ...
                <th className="px-3 py-2 text-start font-semibold">{t("examples.columnProbability")}</th>
              </tr>
            </thead>
            <tbody>
              {exampleRows.map((row) => (
                <tr key={row.scenario} className="border-b border-current/10">
                  ...
                  <td className="px-3 py-2.5 font-semibold">{row.probability}</td>
                </tr>
              ))}
            </tbody>
          </table>
