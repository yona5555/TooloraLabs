...
type ExampleRow = { harmony: string; description: string; example: string };

export default async function ColorPaletteEducation() {
  const t = await getTranslations("tools.color-palette-generator.education");
          ...
          <table className="w-full min-w-[480px] border-collapse text-sm">
            <thead>
              <tr className="border-b border-current/30 text-start">
                <th className="px-3 py-2 text-start font-semibold">{t("examples.columnHarmony")}</th>
                <th className="px-3 py-2 text-start font-semibold">{t("examples.columnDescription")}</th>
                <th className="px-3 py-2 text-start font-semibold">{t("examples.columnExample")}</th>
              </tr>
            </thead>
            <tbody>
              {exampleRows.map((row) => (
                <tr key={row.harmony} className="border-b border-current/10">
                  <td className="px-3 py-2.5 font-semibold">{row.harmony}</td>
                  <td className="px-3 py-2.5">{row.description}</td>
                  <td className="px-3 py-2.5 font-mono">{row.example}</td>
                </tr>
              ))}
            </tbody>
          </table>
