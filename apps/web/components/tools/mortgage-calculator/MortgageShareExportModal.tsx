...
type ScheduleTable = { title: string; columns: string[]; rows: string[][] } | null;

type MortgageShareExportModalProps = {
  mode: MortgageMode;
  inputRows: Row[];
  resultRows: Row[];
  heroLabel: string;
...
export default function MortgageShareExportModal({ mode, inputRows, resultRows, heroLabel, heroValue, sentence, table }: MortgageShareExportModalProps) {
  ...
  async function handleDownloadPdf() {
    ...
    try {
      await generateToolPdf({
        ...
        filename: "mortgage-calculator.pdf",
      });
      setPdfState("done");
  ...
  return (
    <>
      ...
      <div data-print-area className="hidden bg-white p-8 text-black print:block">
        ...
        {table && (
          <>
            <h2 className="mb-2 mt-6 text-sm font-bold text-blue-600">{table.title}</h2>
            <table className="w-full border-collapse text-xs">
              <thead>
                <tr>
                  {table.columns.map((col) => (
                    <th key={col} className="border border-zinc-200 bg-blue-50 px-2 py-1.5 text-start font-semibold text-blue-900">
                      {col}
                    </th>
                  ))}
                </tr>
