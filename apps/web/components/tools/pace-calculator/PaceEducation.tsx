...
type RecordRow = { event: string; menTime: string; menHolder: string; menYear: string; womenTime: string; womenHolder: string; womenYear: string };

export default async function PaceEducation() {
  const t = await getTranslations("tools.pace-calculator.education");

  const exampleRows = t.raw("examples.rows") as ExampleRow[];
  const applicationItems = t.raw("applications.items") as ApplicationItem[];
  const recordRows = t.raw("worldRecords.rows") as RecordRow[];
  ...
  return (
    <EncyclopediaPaper>
      ...
      <InfoSection title={t("finishTimeFormula.title")}>
        <p>{t("finishTimeFormula.paragraph1")}</p>
        <p dir="ltr" className="rounded-sm border border-current/20 p-4 text-center font-mono text-base">
          T₂ = T₁ × (D₂ / D₁)^1.06
        </p>
        <p>{t("finishTimeFormula.paragraph2")}</p>
      </InfoSection>

      <InfoSection title={t("worldRecords.title")}>
        <p>{t("worldRecords.intro")}</p>
        <WorldRecordsTable rows={recordRows} menLabel={t("worldRecords.columnMen")} womenLabel={t("worldRecords.columnWomen")} />
        <p className="text-sm opacity-70">{t("worldRecords.note")}</p>
      </InfoSection>
