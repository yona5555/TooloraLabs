...
export default function AreaShareExportModal({ result, digitStyle, draft }: Props) {
  ...
  const areaText = result.error ? "" : `${fmt(result.area)} ${t("result.squareUnits")}`;
  const shareText = `${t("title")}: ${areaText}`;

  const dimensionFieldsByShape: Record<AreaDraft["shape"], Array<{ key: keyof AreaDraft; label: string }>> = {
    square: [{ key: "side", label: tForm("sideLabel") }],
    rectangle: [
      { key: "width", label: tForm("widthLabel") },
      { key: "height", label: tForm("heightLabel") },
    ],
    triangle: [
      { key: "base", label: tForm("baseLabel") },
      { key: "height", label: tForm("heightLabel") },
    ],
    parallelogram: [
      { key: "base", label: tForm("baseLabel") },
    ...
    circle: [{ key: "radius", label: tForm("radiusLabel") }],
    ellipse: [
      { key: "semiMajorAxis", label: tForm("semiMajorAxisLabel") },
      { key: "semiMinorAxis", label: tForm("semiMinorAxisLabel") },
    ],
    trapezoid: [
      { key: "base1", label: tForm("base1Label") },
      { key: "base2", label: tForm("base2Label") },
      { key: "height", label: tForm("heightLabel") },
    ],
    sector: [
      { key: "radius", label: tForm("radiusLabel") },
      { key: "angleDegrees", label: tForm("angleLabel") },
    ],
  };

  const inputRows = [
    { label: tForm("shapeLabel"), value: tForm(`shape.${draft.shape}`) },
    ...dimensionFieldsByShape[draft.shape].map((f) => ({ label: f.label, value: draft[f.key] || "—" })),
  ];

  const resultRows = result.error ? [] : [{ label: t("result.heading"), value: areaText }];
