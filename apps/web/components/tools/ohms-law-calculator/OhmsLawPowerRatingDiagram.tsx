type OhmsLawPowerRatingDiagramProps = {
  powerWatts: number;
  caption: string;
  ratingLabels: string[];
};

const RATINGS = [0.125, 0.25, 0.5, 1, 2];
const MIN_LOG = -1.2;
const MAX_LOG = 1;
const WIDTH = 320;
const MARGIN = 20;
...
function xForPower(value: number): number {
...
/**
 * Places the calculated power dissipation on a log scale against common
 * fixed resistor wattage ratings (1/8W through 2W) — a practical
 * "which resistor do I actually need" reference, mirroring the
 * material-density comparison scale pattern used elsewhere on the site.
 */
export default function OhmsLawPowerRatingDiagram({ powerWatts, caption, ratingLabels }: OhmsLawPowerRatingDiagramProps) {
  const markerX = xForPower(powerWatts);
