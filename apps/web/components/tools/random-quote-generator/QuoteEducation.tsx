...
export default async function QuoteEducation() {
  const t = await getTranslations("tools.random-quote-generator.education");
  ...
  return (
    <EncyclopediaPaper>
      ...
      <InfoSection id="behind-the-tool" title={t("behindTheTool.title")}>
        <div>
          <h3 className="font-semibold">{t("behindTheTool.sourcing.title")}</h3>
          <p className="mt-2">{t("behindTheTool.sourcing.paragraph")}</p>
        </div>
        <div>
          <h3 className="font-semibold">{t("behindTheTool.misattribution.title")}</h3>
          <p className="mt-2">{t("behindTheTool.misattribution.paragraph")}</p>
        </div>
        <AcademicPathSection
