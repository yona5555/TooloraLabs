...
export default async function LoremEducation() {
  const t = await getTranslations("tools.lorem-ipsum-generator.education");
  ...
  return (
    <EncyclopediaPaper>
      ...
      <InfoSection id="behind-the-tool" title={t("behindTheTool.title")}>
        ...
        <div>
          <h3 className="font-semibold">{t("behindTheTool.rediscovery.title")}</h3>
          <p className="mt-2">{t("behindTheTool.rediscovery.paragraph")}</p>
        </div>
        <AcademicPathSection
