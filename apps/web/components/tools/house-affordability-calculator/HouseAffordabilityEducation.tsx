...
import HouseAffordabilityIncomeChart from "./HouseAffordabilityIncomeChart";

type ExampleRow = { scenario: string; maxHomePrice: string; monthlyPayment: string };
type VariableItem = { name: string; description: string };
type ApplicationItem = { title: string; description: string };

export default async function HouseAffordabilityEducation() {
  const t = await getTranslations("tools.house-affordability-calculator.education");
          ...
          <table className="w-full min-w-[480px] border-collapse text-sm">
            <thead>
              <tr className="border-b border-current/30 text-start">
                ...
                <th className="px-3 py-2 text-start font-semibold">{t("examples.columnMaxHomePrice")}</th>
                <th className="px-3 py-2 text-start font-semibold">{t("examples.columnMonthlyPayment")}</th>
              </tr>
            </thead>
            <tbody>
              {exampleRows.map((row) => (
                <tr key={row.scenario} className="border-b border-current/10">
                  ...
                  <td className="px-3 py-2.5 font-mono font-semibold">{row.maxHomePrice}</td>
                  <td className="px-3 py-2.5 font-mono font-semibold">{row.monthlyPayment}</td>
                </tr>
              ))}
            </tbody>
          </table>
