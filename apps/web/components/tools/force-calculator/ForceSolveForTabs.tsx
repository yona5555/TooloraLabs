...
type ForceSolveForTabsProps<T extends string> = {
  values: T[];
  active: T;
  onChange: (value: T) => void;
  translationKey: "secondLawSolveFor" | "gravitationSolveFor";
};

export default function ForceSolveForTabs<T extends string>({ values, active, onChange, translationKey }: ForceSolveForTabsProps<T>) {
  ...
  return (
    <div role="tablist" aria-label={t("solveForLabel")} className="flex flex-wrap gap-1.5">
      {values.map((value) => (
        ...
        >
          {t(`${translationKey}.${value}` as "secondLawSolveFor.force")}
        </button>
      ))}
    </div>
  );
