...
/**
 * Matches fuel-cost-calculator's FuelWorkedExampleNote / break-even-calculator's
 * BreakEvenWorkedExampleNote exactly — the site-wide reference for the
 * label/value list that sits directly beside a chart (never below it). No
 * `dir="ltr"` on the row itself: that would force the whole label+value row
 * into LTR order regardless of page direction, mirror-inverting the layout
 * on an RTL page. Only the value cell keeps its own `dir="ltr"` for correct
 * digit/currency ordering.
 */
export default function TipWorkedExampleNote({ title, rows }: Props) {
  return (
