...
import { SOLVE_MODES, type SolveMode } from "./types";

type CompoundInterestModeTabsProps = {
  mode: SolveMode;
  onModeChange: (mode: SolveMode) => void;
};

export default function CompoundInterestModeTabs({ mode, onModeChange }: CompoundInterestModeTabsProps) {
  const t = useTranslations("tools.compound-interest-calculator.tabs");

  return (
    <div role="tablist" aria-label={t("groupLabel")} className="flex flex-wrap gap-1.5">
      {SOLVE_MODES.map((value) => (
        <button
          key={value}
          type="button"
          role="tab"
