...
export default async function BodyFatEducation() {
  const t = await getTranslations("tools.body-fat-calculator.education");

  const exampleRows = t.raw("examples.rows") as ExampleRow[];
  const categoryItems = t.raw("categoriesSection.items") as CategoryItem[];
  ...
  return (
    <EncyclopediaPaper>
      ...
      <InfoSection title={t("categoriesSection.title")}>
        <p>{t("categoriesSection.intro")}</p>
