...
export default function MatrixShareExportModal({ result, digitStyle, matrices }: Props) {
  const t = useTranslations("tools.matrix-calculator");
  const tResult = useTranslations("tools.matrix-calculator.result");
  ...
  const matrixText = (m11: number | null, m12: number | null, m21: number | null, m22: number | null) => `[${cell(m11)} ${cell(m12)}; ${cell(m21)} ${cell(m22)}]`;
  const shareUrl = typeof window !== "undefined" ? window.location.href : "";
  const shareText = `${t("title")}: det(A) = ${fmt(result.determinantA)}`;

  const inputRows = [
    { label: "A", value: matrixText(matrices.a11, matrices.a12, matrices.a21, matrices.a22) },
    { label: "B", value: matrixText(matrices.b11, matrices.b12, matrices.b21, matrices.b22) },
  ];

  const resultRows = [
    { label: tResult("sum"), value: matrixText(result.sum11, result.sum12, result.sum21, result.sum22) },
    { label: tResult("difference"), value: matrixText(result.diff11, result.diff12, result.diff21, result.diff22) },
    { label: tResult("product"), value: matrixText(result.product11, result.product12, result.product21, result.product22) },
    { label: tResult("transposeA"), value: matrixText(result.transposeA11, result.transposeA12, result.transposeA21, result.transposeA22) },
    { label: tResult("inverseA"), value: matrixText(result.inverseA11, result.inverseA12, result.inverseA21, result.inverseA22) },
    { label: tResult("determinantA"), value: fmt(result.determinantA) },
    { label: tResult("determinantB"), value: fmt(result.determinantB) },
  ];
  ...
  async function handleDownloadPdf() {
    ...
    try {
      await generateToolPdf({
        ...
        filename: "matrix-calculator.pdf",
      });
      setPdfState("done");
  ...
  return (
    <>
      ...
      <div data-print-area className="hidden bg-white p-8 text-black print:block">
        ...
        <h2 className="mb-2 mt-6 text-sm font-bold text-blue-600">{tResult("heading")}</h2>
        <table className="w-full border-collapse text-sm">
          <tbody>
            {resultRows.map((row) => (
              <tr key={row.label}>
