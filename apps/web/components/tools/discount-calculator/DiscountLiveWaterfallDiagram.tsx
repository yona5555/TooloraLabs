type DiscountLiveWaterfallDiagramProps = {
  stages: { label: string; price: number }[];
...
export default function DiscountLiveWaterfallDiagram({ stages, formatValue, caption }: DiscountLiveWaterfallDiagramProps) {
  const maxPrice = Math.max(...stages.map((s) => s.price), 0.01);
      ...
      <div dir="ltr" className="overflow-x-auto">
        <svg viewBox={`0 0 ${WIDTH} ${height}`} role="img" aria-label={caption} className="h-auto w-full text-current" style={{ minWidth: 280 }}>
          {stages.map((stage, i) => {
            ...
            const width = Math.max((stage.price / maxPrice) * WIDTH, 2);
            return (
              <g key={i}>
