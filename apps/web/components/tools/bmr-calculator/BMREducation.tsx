...
export default async function BMREducation() {
  const t = await getTranslations("tools.bmr-calculator.education");

  const exampleRows = t.raw("examples.rows") as ExampleRow[];
  const formulaItems = t.raw("formulas.items") as FormulaItem[];
  ...
  return (
    <EncyclopediaPaper>
      ...
      <InfoSection title={t("formulas.title")}>
