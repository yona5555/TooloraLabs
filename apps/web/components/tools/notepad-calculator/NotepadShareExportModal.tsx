...
export default function NotepadShareExportModal({ result }: Props) {
  const t = useTranslations("tools.notepad-calculator");
  const tResult = useTranslations("tools.notepad-calculator.result");
  ...
  const calculatedLines = result.lines.filter((line) => line.result !== null);
  const shareUrl = typeof window !== "undefined" ? window.location.href : "";
  const lastResult = calculatedLines.length > 0 ? calculatedLines[calculatedLines.length - 1].result : "";
  const shareText = `${t("title")}: ${lastResult}`;

  const linesTable = {
    title: t("shareExport.linesTableTitle"),
    columns: [t("shareExport.lineColumn"), t("shareExport.resultColumn")],
    rows: result.lines.filter((line) => line.text.trim().length > 0).map((line) => [line.text, line.result !== null ? String(line.result) : "—"]),
  };
  ...
  async function handleDownloadPdf() {
    ...
    try {
      await generateToolPdf({
        ...
        toolName: t("title"),
        inputs: [],
        results: calculatedLines.length > 0 ? [{ label: tResult("calculatedCount", { count: calculatedLines.length }), value: String(lastResult) }] : [],
        table: linesTable,
        preparedFor: preparedForRows,
        preparedForTitle: t("shareExport.preparedForTitle"),
        brandingEnhancements: true,
        filename: "notepad-calculator.pdf",
      });
      setPdfState("done");
  ...
  return (
    <>
      ...
      <div data-print-area className="hidden bg-white p-8 text-black print:block">
        ...
        <h2 className="mb-2 mt-6 text-sm font-bold text-blue-600">{linesTable.title}</h2>
        <table className="w-full border-collapse text-sm">
          <thead>
            <tr>
              {linesTable.columns.map((col) => (
