...
import QRAnatomyDiagram from "./QRAnatomyDiagram";

type ExampleRow = { scenario: string; result: string };

export default async function QREducation() {
  const t = await getTranslations("tools.qr-code-generator.education");
  ...
  return (
    <EncyclopediaPaper>
      <InfoSection title={t("intro.title")}>
        ...
        <QRAnatomyDiagram
          finderLabel={t("intro.diagram.finderLabel")}
          timingLabel={t("intro.diagram.timingLabel")}
          quietZoneLabel={t("intro.diagram.quietZoneLabel")}
          dataLabel={t("intro.diagram.dataLabel")}
            ...
            <tbody>
              {exampleRows.map((row) => (
                <tr key={row.scenario} className="border-b border-current/10">
                  ...
                  <td className="break-all px-3 py-2.5 font-mono text-xs font-semibold">{row.result}</td>
                </tr>
              ))}
            </tbody>
          </table>
