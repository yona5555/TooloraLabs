...
type WorldTimeShareExportModalProps = {
  inputRows: Row[];
  resultRows: Row[];
  heroLabel: string;
  heroValue: string;
...
export default function WorldTimeShareExportModal({ inputRows, resultRows, heroLabel, heroValue, sentence, gauge }: WorldTimeShareExportModalProps) {
  const t = useTranslations("tools.world-time-converter");
  ...
  async function handleDownloadPdf() {
    ...
    try {
      await generateToolPdf({
        ...
        filename: "world-time-converter.pdf",
      });
      setPdfState("done");
  ...
  return (
    <>
      <button
        ...
        className="flex items-center gap-1.5 rounded-lg border border-current/20 px-3 py-1.5 text-xs font-medium text-current/80 transition hover:bg-current/5 dark:border-zinc-700"
      >
        <Share2 size={14} />
      ...
      <div data-print-area className="hidden bg-white p-8 text-black print:block">
        ...
        <h2 className="mb-2 mt-6 text-sm font-bold text-blue-600">{t("aboveFold.converterTitle")}</h2>
