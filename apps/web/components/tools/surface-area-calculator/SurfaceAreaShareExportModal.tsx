...
import type { Solid3DDraft, SurfaceAreaResult as Result } from "./types";
...
export default function SurfaceAreaShareExportModal({ result, digitStyle, draft }: Props) {
  ...
  const areaText = result.error ? "" : `${fmt(result.surfaceArea)} ${t("result.squareUnits")}`;
  const shareText = `${t("title")}: ${areaText}`;

  const dimensionFieldsByShape: Record<Solid3DDraft["shape"], Array<{ key: keyof Solid3DDraft; label: string }>> = {
    cube: [{ key: "side", label: tForm("sideLabel") }],
    "rectangular-prism": [
      { key: "length", label: tForm("lengthLabel") },
      { key: "width", label: tForm("widthLabel") },
      { key: "height", label: tForm("heightLabel") },
    ],
    sphere: [{ key: "radius", label: tForm("radiusLabel") }],
    cylinder: [
    ...
    "square-pyramid": [
      { key: "baseSide", label: tForm("baseSideLabel") },
      { key: "height", label: tForm("heightLabel") },
    ],
  };
  ...
  async function handleDownloadPdf() {
    ...
    try {
      await generateToolPdf({
        ...
        filename: "surface-area-calculator.pdf",
      });
      setPdfState("done");
