...
export default function FunctionCategoryRow({ dispatch }: FunctionCategoryRowProps) {
  ...
  return (
    <div className="border-t border-zinc-200 calcdark:border-zinc-800">
      <div className="flex">
        {categories.map((key) => {
          ...
          return (
            <button
              ...
              className={`flex flex-1 items-center justify-center gap-1.5 border-e border-zinc-200 px-2 py-2 text-xs font-semibold transition last:border-e-0 sm:text-sm calcdark:border-zinc-800 ${
                isOpen ? "bg-blue-50 text-blue-600 calcdark:bg-blue-500/10 calcdark:text-blue-400" : "text-zinc-600 hover:bg-zinc-50 calcdark:text-zinc-300 calcdark:hover:bg-zinc-800/60"
              }`}
            >
              <Icon size={14} />
      ...
      {open === "trigonometry" && (
        <div className="flex flex-wrap gap-1.5 bg-zinc-50 px-3 py-2.5 calcdark:bg-zinc-800/40" dir="ltr">
          {TRIG_BUTTONS.map((label) => (
            <button
              key={label}
              type="button"
              onClick={() => pressFunction(label)}
              className="rounded-lg border border-zinc-200 bg-white px-2.5 py-1 text-sm font-medium text-zinc-700 transition hover:bg-zinc-100 calcdark:border-zinc-700 calcdark:bg-zinc-900 calcdark:text-zinc-200 calcdark:hover:bg-zinc-800"
            >
              {label}
            </button>
          ))}
      ...
      {open === "probability" && (
        <div className="bg-zinc-50 px-3 py-2.5 calcdark:bg-zinc-800/40">
          ...
          <p className="mt-2 text-xs text-zinc-400 calcdark:text-zinc-500">{tHome("functionCategories.probabilityHint")}</p>
        </div>
      )}
      ...
      {open === "calculus" && (
        <div className="bg-zinc-50 px-3 py-2.5 calcdark:bg-zinc-800/40">
          ...
          <p className="mt-2 text-xs text-zinc-400 calcdark:text-zinc-500">{tHome("functionCategories.calculusHint")}</p>
        </div>
      )}
    </div>
  );
