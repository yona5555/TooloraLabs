...
type HomeShareExportModalProps = {
  history: HistoryEntry[];
  currentDisplay: string;
};

/**
 * Adapted from ScientificShareExportModal (the site's standard Share/Export
 * pattern — PDF/print/social/copy-link/Prepared-For) but built around a full
 * operation history instead of a single input/result pair: generateToolPdf's
 * `table` option already supports an arbitrary multi-row table, so the whole
 * log becomes one PDF table rather than a single result row.
 */
export default function HomeShareExportModal({ history, currentDisplay }: HomeShareExportModalProps) {
  ...
  const shareText = t("shareExport.summarySentence", { value: currentDisplay });
  ...
  const historyRows = history.map((entry) => [entry.expression, formatCalculatorNumber(entry.result)]);
  ...
  async function handleDownloadPdf() {
    ...
    try {
      await generateToolPdf({
        locale,
        toolName: tHome("shareExport.title"),
        inputs: [],
        results: [{ label: t("shareExport.currentResultLabel"), value: currentDisplay }],
        table:
          historyRows.length > 0
            ? {
                title: tHome("shareExport.historyTableTitle"),
                columns: [tHome("shareExport.expressionColumn"), tHome("shareExport.resultColumn")],
                rows: historyRows,
              }
            : undefined,
        sentence: shareText,
        sentenceTitle: t("shareExport.howCalculatedTitle"),
        preparedFor: preparedForRows,
        preparedForTitle: t("shareExport.preparedForTitle"),
        brandingEnhancements: true,
        filename: "scientific-calculator.pdf",
      });
      setPdfState("done");
  ...
  return (
    <>
      ...
      <div data-print-area className="hidden bg-white p-8 text-black print:block">
        ...
        <h1 className="mb-4 text-2xl font-bold text-zinc-900">{tHome("shareExport.title")}</h1>

        <h2 className="mb-2 mt-6 text-base font-bold text-blue-600">{t("shareExport.resultSectionTitle")}</h2>
        <table className="w-full border-collapse text-base">
          <tbody>
            <tr>
              <td className="border border-zinc-200 bg-blue-50 px-3 py-2 font-semibold text-blue-900">{t("shareExport.currentResultLabel")}</td>
              <td className="border border-zinc-200 px-3 py-2 font-semibold">{currentDisplay}</td>
            </tr>
          </tbody>
        </table>
        ...
        {historyRows.length > 0 && (
          <>
            <h2 className="mb-2 mt-6 text-base font-bold text-blue-600">{tHome("shareExport.historyTableTitle")}</h2>
            <table className="w-full border-collapse text-base">
              <thead>
                <tr>
                  <th className="border border-zinc-200 bg-blue-50 px-3 py-2 text-start font-semibold text-blue-900">{tHome("shareExport.expressionColumn")}</th>
                  <th className="border border-zinc-200 bg-blue-50 px-3 py-2 text-start font-semibold text-blue-900">{tHome("shareExport.resultColumn")}</th>
                </tr>
              </thead>
              <tbody>
                {historyRows.map((row, i) => (
                  <tr key={i}>
                    <td className="border border-zinc-200 px-3 py-2">{row[0]}</td>
                    <td className="border border-zinc-200 px-3 py-2">{row[1]}</td>
                  </tr>
                ))}
              </tbody>
            </table>
