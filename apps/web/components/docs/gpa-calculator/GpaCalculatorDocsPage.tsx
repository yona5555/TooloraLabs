...
export const RELATED_TOOLS = ["study-time-calculator", "citation-generator", "class-schedule-builder"];

export async function getGpaCalculatorTocItems(): Promise<TocItem[]> {
  const t = await getTranslations("docs.tools.gpa-calculator");
  return [
...
export default async function GpaCalculatorDocsPage() {
  const t = await getTranslations("docs.tools.gpa-calculator");
  ...
  return (
    <>
      <DocsBreadcrumb items={[{ label: tNav("overview"), href: "/docs" }, { label: tNav("toolsGuide"), href: "/docs" }, { label: tTools("gpa-calculator.title") }]} />
      <DocsHero title={tTools("gpa-calculator.title")} version={t("version")} description={t("description")} />
      ...
      <DocsSection id="anatomy" title={t("sectionAnatomy")}>
        ...
        <BreakdownBarDiagram
          segments={[
            { label: t("anatomy.labelCourseA"), value: 12, colorClass: "fill-blue-600 dark:fill-blue-400" },
            { label: t("anatomy.labelCourseB"), value: 12, colorClass: "fill-emerald-500 dark:fill-emerald-400" },
          ]}
