...
import DigitHighlightDiagram from "@/components/docs/DigitHighlightDiagram";
import ValueComparisonBarDiagram from "@/components/docs/ValueComparisonBarDiagram";

export const RELATED_TOOLS = ["scientific-notation-converter", "fraction-calculator", "percentage-calculator", "scientific-calculator", "standard-deviation-calculator"];

export async function getSignificantFiguresCalculatorTocItems(): Promise<TocItem[]> {
  const t = await getTranslations("docs.tools.significant-figures-calculator");
  return [
    { id: "formula", label: t("sectionFormula") },
    { id: "flowchart", label: t("sectionFlowchart") },
    { id: "digits", label: t("sectionDigits") },
...
const SAMPLE_DIGITS = [
  { char: "1", significant: true },
  { char: "0", significant: true },
  { char: "0", significant: true },
  { char: ".", significant: false },
  { char: "2", significant: true },
  { char: "0", significant: true },
];

export default async function SignificantFiguresCalculatorDocsPage() {
  const t = await getTranslations("docs.tools.significant-figures-calculator");
  ...
  return (
    <>
      <DocsBreadcrumb items={[{ label: tNav("overview"), href: "/docs" }, { label: tNav("toolsGuide"), href: "/docs" }, { label: tTools("significant-figures-calculator.title") }]} />
      <DocsHero title={tTools("significant-figures-calculator.title")} version={t("version")} description={t("description")} />
      ...
      <DocsSection id="digits" title={t("sectionDigits")}>
        <p className="mb-4 text-zinc-600 dark:text-zinc-300">{t("digits.intro")}</p>
        <DigitHighlightDiagram
          digits={SAMPLE_DIGITS}
          significantLabel={t("digits.significantLabel")}
          notSignificantLabel={t("digits.notSignificantLabel")}
          caption={t("digits.caption")}
        />
      </DocsSection>
      ...
      <DocsSection id="anatomy" title={t("sectionAnatomy")}>
        ...
        <ValueComparisonBarDiagram
          bars={[
            { label: t("anatomy.labelInputA"), value: 4, colorClass: "fill-blue-600 dark:fill-blue-400" },
            { label: t("anatomy.labelInputB"), value: 2, colorClass: "fill-blue-400 dark:fill-blue-300" },
            { label: t("anatomy.labelResult"), value: 2, colorClass: "fill-emerald-500 dark:fill-emerald-400" },
          ]}
