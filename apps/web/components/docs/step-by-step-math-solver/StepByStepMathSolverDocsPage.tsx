...
export const RELATED_TOOLS = ["scientific-calculator", "graphing-calculator", "matrix-calculator", "fraction-calculator", "percentage-calculator"];

export async function getStepByStepMathSolverTocItems(): Promise<TocItem[]> {
  const t = await getTranslations("docs.tools.step-by-step-math-solver");
  return [
    { id: "formula", label: t("sectionFormula") },
    { id: "flowchart", label: t("sectionFlowchart") },
    { id: "roots", label: t("sectionRoots") },
...
export default async function StepByStepMathSolverDocsPage() {
  const t = await getTranslations("docs.tools.step-by-step-math-solver");
  ...
  return (
    <>
      <DocsBreadcrumb items={[{ label: tNav("overview"), href: "/docs" }, { label: tNav("toolsGuide"), href: "/docs" }, { label: tTools("step-by-step-math-solver.title") }]} />
      <DocsHero title={tTools("step-by-step-math-solver.title")} version={t("version")} description={t("description")} />
      ...
      <DocsSection id="roots" title={t("sectionRoots")}>
        <p className="mb-4 text-zinc-600 dark:text-zinc-300">{t("roots.intro")}</p>
        <DotPlotDiagram
          values={[2, 3]}
          markers={[
            { value: 2, label: t("roots.labelX1"), colorClass: "stroke-blue-600 dark:stroke-blue-400 fill-blue-600 dark:fill-blue-400" },
            { value: 3, label: t("roots.labelX2"), colorClass: "stroke-purple-500 dark:stroke-purple-400 fill-purple-500 dark:fill-purple-400" },
          ]}
          caption={t("roots.caption")}
        />
      </DocsSection>

      <DocsSection id="anatomy" title={t("sectionAnatomy")}>
        <p className="mb-4 text-zinc-600 dark:text-zinc-300">{t("anatomy.intro")}</p>
        <ScalingCurveDiagram points={PARABOLA_POINTS} xLabel={t("anatomy.xLabel")} yLabel={t("anatomy.yLabel")} caption={t("anatomy.caption")} />
      </DocsSection>
