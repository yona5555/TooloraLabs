...
export const RELATED_TOOLS = ["study-time-calculator", "gpa-calculator", "citation-generator"];

export async function getClassScheduleBuilderTocItems(): Promise<TocItem[]> {
  const t = await getTranslations("docs.tools.class-schedule-builder");
  return [
...
export default async function ClassScheduleBuilderDocsPage() {
  const t = await getTranslations("docs.tools.class-schedule-builder");
  ...
  return (
    <>
      <DocsBreadcrumb items={[{ label: tNav("overview"), href: "/docs" }, { label: tNav("toolsGuide"), href: "/docs" }, { label: tTools("class-schedule-builder.title") }]} />
      <DocsHero title={tTools("class-schedule-builder.title")} version={t("version")} description={t("description")} />
      ...
      <DocsSection id="anatomy" title={t("sectionAnatomy")}>
        ...
        <BreakdownBarDiagram
          segments={[
            { label: t("anatomy.labelAOnly"), value: 60, colorClass: "fill-blue-600 dark:fill-blue-400" },
            { label: t("anatomy.labelOverlap"), value: 30, colorClass: "fill-red-500 dark:fill-red-400" },
            { label: t("anatomy.labelBOnly"), value: 30, colorClass: "fill-emerald-500 dark:fill-emerald-400" },
          ]}
