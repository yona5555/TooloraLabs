...
export const RELATED_TOOLS = ["stoichiometry-calculator", "chemical-equation-balancer", "molarity-calculator"];

export async function getMolarMassCalculatorTocItems(): Promise<TocItem[]> {
  const t = await getTranslations("docs.tools.molar-mass-calculator");
  return [
...
export default async function MolarMassCalculatorDocsPage() {
  const t = await getTranslations("docs.tools.molar-mass-calculator");
  ...
  return (
    <>
      <DocsBreadcrumb items={[{ label: tNav("overview"), href: "/docs" }, { label: tNav("toolsGuide"), href: "/docs" }, { label: tTools("molar-mass-calculator.title") }]} />
      <DocsHero title={tTools("molar-mass-calculator.title")} version={t("version")} description={t("description")} />
      ...
      <DocsSection id="anatomy" title={t("sectionAnatomy")}>
        ...
        <BreakdownBarDiagram
          segments={[
            { label: t("anatomy.labelC"), value: 72.066, colorClass: "fill-blue-600 dark:fill-blue-400" },
            { label: t("anatomy.labelH"), value: 12.096, colorClass: "fill-emerald-500 dark:fill-emerald-400" },
            { label: t("anatomy.labelO"), value: 95.994, colorClass: "fill-orange-500 dark:fill-orange-400" },
          ]}
