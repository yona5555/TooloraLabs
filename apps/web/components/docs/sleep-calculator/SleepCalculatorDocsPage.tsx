...
export const RELATED_TOOLS = ["target-heart-rate-calculator", "tdee-calculator", "pace-calculator", "bmr-calculator", "macro-calculator"];

export async function getSleepCalculatorTocItems(): Promise<TocItem[]> {
  const t = await getTranslations("docs.tools.sleep-calculator");
  return [
...
export default async function SleepCalculatorDocsPage() {
  const t = await getTranslations("docs.tools.sleep-calculator");
  ...
  return (
    <>
      <DocsBreadcrumb items={[{ label: tNav("overview"), href: "/docs" }, { label: tNav("toolsGuide"), href: "/docs" }, { label: tTools("sleep-calculator.title") }]} />
      <DocsHero title={tTools("sleep-calculator.title")} version={t("version")} description={t("description")} />
      ...
      <DocsSection id="anatomy" title={t("sectionAnatomy")}>
        ...
        <ValueComparisonBarDiagram
          bars={[
            { label: t("anatomy.label6"), value: 9, colorClass: "fill-blue-600 dark:fill-blue-400" },
            { label: t("anatomy.label5"), value: 7.5, colorClass: "fill-emerald-500 dark:fill-emerald-400" },
            { label: t("anatomy.label4"), value: 6, colorClass: "fill-amber-500 dark:fill-amber-400" },
            { label: t("anatomy.label3"), value: 4.5, colorClass: "fill-purple-500 dark:fill-purple-400" },
          ]}
