...
export const RELATED_TOOLS = ["qr-code-generator", "color-palette-generator", "json-formatter"];

export async function getBarcodeGeneratorTocItems(): Promise<TocItem[]> {
  const t = await getTranslations("docs.tools.barcode-generator");
  return [
...
export default async function BarcodeGeneratorDocsPage() {
  const t = await getTranslations("docs.tools.barcode-generator");
  ...
  return (
    <>
      <DocsBreadcrumb items={[{ label: tNav("overview"), href: "/docs" }, { label: tNav("toolsGuide"), href: "/docs" }, { label: tTools("barcode-generator.title") }]} />
      <DocsHero title={tTools("barcode-generator.title")} version={t("version")} description={t("description")} />
      ...
      <DocsSection id="anatomy" title={t("sectionAnatomy")}>
        ...
        <ValueComparisonBarDiagram
          bars={[
            { label: t("anatomy.labelUpcA"), value: 11, colorClass: "fill-blue-600 dark:fill-blue-400" },
            { label: t("anatomy.labelEan13"), value: 12, colorClass: "fill-emerald-500 dark:fill-emerald-400" },
            { label: t("anatomy.labelCode128"), value: 48, colorClass: "fill-purple-500 dark:fill-purple-400" },
          ]}
