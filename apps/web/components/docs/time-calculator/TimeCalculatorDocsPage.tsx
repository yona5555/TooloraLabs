...
export const RELATED_TOOLS = ["date-calculator", "world-time-converter", "countdown-to-event-calculator"];

export async function getTimeCalculatorTocItems(): Promise<TocItem[]> {
  const t = await getTranslations("docs.tools.time-calculator");
  return [
...
export default async function TimeCalculatorDocsPage() {
  const t = await getTranslations("docs.tools.time-calculator");
  ...
  return (
    <>
      <DocsBreadcrumb items={[{ label: tNav("overview"), href: "/docs" }, { label: tNav("toolsGuide"), href: "/docs" }, { label: tTools("time-calculator.title") }]} />
      <DocsHero title={tTools("time-calculator.title")} version={t("version")} description={t("description")} />
      ...
      <DocsSection id="anatomy" title={t("sectionAnatomy")}>
        ...
        <BreakdownBarDiagram
          segments={[
            { label: t("anatomy.labelHours"), value: 7200, colorClass: "fill-blue-600 dark:fill-blue-400" },
            { label: t("anatomy.labelMinutes"), value: 900, colorClass: "fill-emerald-500 dark:fill-emerald-400" },
            { label: t("anatomy.labelSeconds"), value: 30, colorClass: "fill-purple-500 dark:fill-purple-400" },
          ]}
