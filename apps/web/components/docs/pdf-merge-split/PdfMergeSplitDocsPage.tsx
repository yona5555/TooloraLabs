...
export const RELATED_TOOLS = ["zip-compressor", "csv-json-converter", "file-size-converter"];

export async function getPdfMergeSplitTocItems(): Promise<TocItem[]> {
  const t = await getTranslations("docs.tools.pdf-merge-split");
  return [
...
export default async function PdfMergeSplitDocsPage() {
  const t = await getTranslations("docs.tools.pdf-merge-split");
  ...
  return (
    <>
      <DocsBreadcrumb items={[{ label: tNav("overview"), href: "/docs" }, { label: tNav("toolsGuide"), href: "/docs" }, { label: tTools("pdf-merge-split.title") }]} />
      <DocsHero title={tTools("pdf-merge-split.title")} version={t("version")} description={t("description")} />
      ...
      <DocsSection id="anatomy" title={t("sectionAnatomy")}>
        ...
        <BreakdownBarDiagram
          segments={[
            { label: t("anatomy.labelRangeOne"), value: 3, colorClass: "fill-blue-600 dark:fill-blue-400" },
            { label: t("anatomy.labelSingle"), value: 1, colorClass: "fill-emerald-500 dark:fill-emerald-400" },
            { label: t("anatomy.labelRangeTwo"), value: 2, colorClass: "fill-purple-500 dark:fill-purple-400" },
          ]}
