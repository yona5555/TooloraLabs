...
export const RELATED_TOOLS = ["tdee-calculator", "pace-calculator", "bmr-calculator", "macro-calculator", "sleep-calculator"];

export async function getTargetHeartRateCalculatorTocItems(): Promise<TocItem[]> {
  const t = await getTranslations("docs.tools.target-heart-rate-calculator");
  return [
...
export default async function TargetHeartRateCalculatorDocsPage() {
  const t = await getTranslations("docs.tools.target-heart-rate-calculator");
  ...
  return (
    <>
      <DocsBreadcrumb items={[{ label: tNav("overview"), href: "/docs" }, { label: tNav("toolsGuide"), href: "/docs" }, { label: tTools("target-heart-rate-calculator.title") }]} />
      <DocsHero title={tTools("target-heart-rate-calculator.title")} version={t("version")} description={t("description")} />
      ...
      <DocsSection id="anatomy" title={t("sectionAnatomy")}>
        ...
        <ValueComparisonBarDiagram
          bars={[
            { label: t("anatomy.labelVeryLight"), value: 114, colorClass: "fill-blue-400 dark:fill-blue-300" },
            { label: t("anatomy.labelLight"), value: 133, colorClass: "fill-emerald-500 dark:fill-emerald-400" },
            { label: t("anatomy.labelModerate"), value: 152, colorClass: "fill-amber-500 dark:fill-amber-400" },
            { label: t("anatomy.labelHard"), value: 171, colorClass: "fill-orange-500 dark:fill-orange-400" },
            { label: t("anatomy.labelMaximum"), value: 190, colorClass: "fill-red-500 dark:fill-red-400" },
          ]}
