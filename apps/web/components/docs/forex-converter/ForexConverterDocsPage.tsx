...
export const RELATED_TOOLS = ["crypto-converter", "commodities-tracker", "unit-converter"];

export async function getForexConverterTocItems(): Promise<TocItem[]> {
  const t = await getTranslations("docs.tools.forex-converter");
  return [
...
export default async function ForexConverterDocsPage() {
  const t = await getTranslations("docs.tools.forex-converter");
  ...
  return (
    <>
      <DocsBreadcrumb items={[{ label: tNav("overview"), href: "/docs" }, { label: tNav("toolsGuide"), href: "/docs" }, { label: tTools("forex-converter.title") }]} />
      <DocsHero title={tTools("forex-converter.title")} version={t("version")} description={t("description")} />
