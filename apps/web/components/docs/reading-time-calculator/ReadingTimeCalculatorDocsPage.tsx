...
export const RELATED_TOOLS = ["word-counter", "speech-word-count-calculator", "readability-score-calculator"];

export async function getReadingTimeCalculatorTocItems(): Promise<TocItem[]> {
  const t = await getTranslations("docs.tools.reading-time-calculator");
  return [
...
export default async function ReadingTimeCalculatorDocsPage() {
  const t = await getTranslations("docs.tools.reading-time-calculator");
  ...
  return (
    <>
      <DocsBreadcrumb items={[{ label: tNav("overview"), href: "/docs" }, { label: tNav("toolsGuide"), href: "/docs" }, { label: tTools("reading-time-calculator.title") }]} />
      <DocsHero title={tTools("reading-time-calculator.title")} version={t("version")} description={t("description")} />
      ...
      <DocsSection id="anatomy" title={t("sectionAnatomy")}>
        ...
        <ValueComparisonBarDiagram
          bars={[
            { label: t("anatomy.labelSlow"), value: 200, colorClass: "fill-blue-600 dark:fill-blue-400" },
            { label: t("anatomy.labelAverage"), value: 130, colorClass: "fill-emerald-500 dark:fill-emerald-400" },
            { label: t("anatomy.labelFast"), value: 100, colorClass: "fill-purple-500 dark:fill-purple-400" },
          ]}
