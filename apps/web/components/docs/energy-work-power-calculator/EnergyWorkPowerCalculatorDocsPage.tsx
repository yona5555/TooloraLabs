...
export const RELATED_TOOLS = ["ohms-law-calculator", "force-calculator", "kinematics-calculator"];

export async function getEnergyWorkPowerCalculatorTocItems(): Promise<TocItem[]> {
  const t = await getTranslations("docs.tools.energy-work-power-calculator");
  return [
...
export default async function EnergyWorkPowerCalculatorDocsPage() {
  const t = await getTranslations("docs.tools.energy-work-power-calculator");
  ...
  return (
    <>
      <DocsBreadcrumb items={[{ label: tNav("overview"), href: "/docs" }, { label: tNav("toolsGuide"), href: "/docs" }, { label: tTools("energy-work-power-calculator.title") }]} />
      <DocsHero title={tTools("energy-work-power-calculator.title")} version={t("version")} description={t("description")} />
