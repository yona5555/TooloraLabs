...
export const RELATED_TOOLS = ["readability-score-calculator", "reading-time-calculator", "speech-word-count-calculator"];

export async function getWordCounterTocItems(): Promise<TocItem[]> {
  const t = await getTranslations("docs.tools.word-counter");
  return [
...
export default async function WordCounterDocsPage() {
  const t = await getTranslations("docs.tools.word-counter");
  ...
  return (
    <>
      <DocsBreadcrumb items={[{ label: tNav("overview"), href: "/docs" }, { label: tNav("toolsGuide"), href: "/docs" }, { label: tTools("word-counter.title") }]} />
      <DocsHero title={tTools("word-counter.title")} version={t("version")} description={t("description")} />
      ...
      <DocsSection id="anatomy" title={t("sectionAnatomy")}>
        ...
        <ValueComparisonBarDiagram
          bars={[
            { label: t("anatomy.labelWord1"), value: 5, colorClass: "fill-blue-600 dark:fill-blue-400" },
            { label: t("anatomy.labelWord2"), value: 4, colorClass: "fill-emerald-500 dark:fill-emerald-400" },
            { label: t("anatomy.labelWord3"), value: 3, colorClass: "fill-purple-500 dark:fill-purple-400" },
            { label: t("anatomy.labelWord4"), value: 2, colorClass: "fill-amber-500 dark:fill-amber-400" },
            { label: t("anatomy.labelWord5"), value: 2, colorClass: "fill-zinc-400 dark:fill-zinc-600" },
          ]}
