...
export const RELATED_TOOLS = ["base64-tool", "json-formatter", "qr-code-generator"];

export async function getPasswordGeneratorTocItems(): Promise<TocItem[]> {
  const t = await getTranslations("docs.tools.password-generator");
  return [
...
export default async function PasswordGeneratorDocsPage() {
  const t = await getTranslations("docs.tools.password-generator");
  ...
  return (
    <>
      <DocsBreadcrumb items={[{ label: tNav("overview"), href: "/docs" }, { label: tNav("toolsGuide"), href: "/docs" }, { label: tTools("password-generator.title") }]} />
      <DocsHero title={tTools("password-generator.title")} version={t("version")} description={t("description")} />
