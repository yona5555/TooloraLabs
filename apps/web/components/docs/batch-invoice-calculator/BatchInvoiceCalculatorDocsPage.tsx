...
export const RELATED_TOOLS = ["invoice-generator", "sales-tax-calculator", "inventory-valuation-calculator", "discount-calculator", "break-even-calculator"];

export async function getBatchInvoiceCalculatorTocItems(): Promise<TocItem[]> {
  const t = await getTranslations("docs.tools.batch-invoice-calculator");
  return [
...
export default async function BatchInvoiceCalculatorDocsPage() {
  const t = await getTranslations("docs.tools.batch-invoice-calculator");
  ...
  return (
    <>
      <DocsBreadcrumb items={[{ label: tNav("overview"), href: "/docs" }, { label: tNav("toolsGuide"), href: "/docs" }, { label: tTools("batch-invoice-calculator.title") }]} />
      <DocsHero title={tTools("batch-invoice-calculator.title")} version={t("version")} description={t("description")} />
      ...
      <DocsSection id="anatomy" title={t("sectionAnatomy")}>
        ...
        <BreakdownBarDiagram
          segments={[
            { label: t("anatomy.labelSubtotal"), value: 250, colorClass: "fill-blue-600 dark:fill-blue-400" },
            { label: t("anatomy.labelTax"), value: 25, colorClass: "fill-amber-500 dark:fill-amber-400" },
          ]}
