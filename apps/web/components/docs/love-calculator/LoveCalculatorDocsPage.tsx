...
export const RELATED_TOOLS = ["random-quote-generator", "dice-roller", "gpa-calculator"];

export async function getLoveCalculatorTocItems(): Promise<TocItem[]> {
  const t = await getTranslations("docs.tools.love-calculator");
  return [
    { id: "formula", label: t("sectionFormula") },
    { id: "flowchart", label: t("sectionFlowchart") },
    { id: "overlap", label: t("sectionOverlap") },
...
export default async function LoveCalculatorDocsPage() {
  const t = await getTranslations("docs.tools.love-calculator");
  ...
  return (
    <>
      <DocsBreadcrumb items={[{ label: tNav("overview"), href: "/docs" }, { label: tNav("toolsGuide"), href: "/docs" }, { label: tTools("love-calculator.title") }]} />
      <DocsHero title={tTools("love-calculator.title")} version={t("version")} description={t("description")} />
      ...
      <DocsSection id="overlap" title={t("sectionOverlap")}>
        <p className="mb-4 text-zinc-600 dark:text-zinc-300">{t("overlap.intro")}</p>
        <VennOverlapDiagram
          leftLabel={t("overlap.leftLabel")}
          rightLabel={t("overlap.rightLabel")}
          overlapLabel={t("overlap.overlapLabel")}
          leftOnlyItems={t("overlap.leftOnlyItems")}
          overlapItems={t("overlap.overlapItems")}
          rightOnlyItems={t("overlap.rightOnlyItems")}
          caption={t("overlap.caption")}
        />
      </DocsSection>

      <DocsSection id="anatomy" title={t("sectionAnatomy")}>
        <p className="mb-4 text-zinc-600 dark:text-zinc-300">{t("anatomy.intro")}</p>
        <PercentGridDiagram percent={73} caption={t("anatomy.caption")} />
      </DocsSection>
