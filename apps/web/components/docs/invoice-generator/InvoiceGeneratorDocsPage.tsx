...
export const RELATED_TOOLS = ["batch-invoice-calculator", "sales-tax-calculator", "discount-calculator", "inventory-valuation-calculator", "break-even-calculator"];

export async function getInvoiceGeneratorTocItems(): Promise<TocItem[]> {
  const t = await getTranslations("docs.tools.invoice-generator");
  return [
...
export default async function InvoiceGeneratorDocsPage() {
  const t = await getTranslations("docs.tools.invoice-generator");
  ...
  return (
    <>
      <DocsBreadcrumb items={[{ label: tNav("overview"), href: "/docs" }, { label: tNav("toolsGuide"), href: "/docs" }, { label: tTools("invoice-generator.title") }]} />
      <DocsHero title={tTools("invoice-generator.title")} version={t("version")} description={t("description")} />
      ...
      <DocsSection id="anatomy" title={t("sectionAnatomy")}>
        ...
        <BreakdownBarDiagram
          segments={[
            { label: t("anatomy.labelTaxable"), value: 90, colorClass: "fill-blue-600 dark:fill-blue-400" },
            { label: t("anatomy.labelTax"), value: 9, colorClass: "fill-amber-500 dark:fill-amber-400" },
          ]}
