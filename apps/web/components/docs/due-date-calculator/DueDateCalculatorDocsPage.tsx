...
export const RELATED_TOOLS = ["pregnancy-calculator", "ovulation-calculator", "date-calculator", "age-calculator", "countdown-to-event-calculator"];

export async function getDueDateCalculatorTocItems(): Promise<TocItem[]> {
  const t = await getTranslations("docs.tools.due-date-calculator");
  return [
...
export default async function DueDateCalculatorDocsPage() {
  const t = await getTranslations("docs.tools.due-date-calculator");
  ...
  return (
    <>
      <DocsBreadcrumb items={[{ label: tNav("overview"), href: "/docs" }, { label: tNav("toolsGuide"), href: "/docs" }, { label: tTools("due-date-calculator.title") }]} />
      <DocsHero title={tTools("due-date-calculator.title")} version={t("version")} description={t("description")} />
      ...
      <DocsSection id="anatomy" title={t("sectionAnatomy")}>
        ...
        <DotPlotDiagram
          values={[0, 14, 280]}
          markers={[
            { value: 0, label: t("anatomy.labelLmp"), colorClass: "stroke-blue-600 dark:stroke-blue-400 fill-blue-600 dark:fill-blue-400" },
            { value: 14, label: t("anatomy.labelConception"), colorClass: "stroke-emerald-500 dark:stroke-emerald-400 fill-emerald-500 dark:fill-emerald-400" },
            { value: 280, label: t("anatomy.labelDue"), colorClass: "stroke-purple-500 dark:stroke-purple-400 fill-purple-500 dark:fill-purple-400" },
          ]}
