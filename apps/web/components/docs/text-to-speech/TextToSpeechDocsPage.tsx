...
export const RELATED_TOOLS = ["speech-to-text", "spelling-grammar-checker", "word-counter"];

export async function getTextToSpeechTocItems(): Promise<TocItem[]> {
  const t = await getTranslations("docs.tools.text-to-speech");
  return [
...
export default async function TextToSpeechDocsPage() {
  const t = await getTranslations("docs.tools.text-to-speech");
  ...
  return (
    <>
      <DocsBreadcrumb items={[{ label: tNav("overview"), href: "/docs" }, { label: tNav("toolsGuide"), href: "/docs" }, { label: tTools("text-to-speech.title") }]} />
      <DocsHero title={tTools("text-to-speech.title")} version={t("version")} description={t("description")} />
      ...
      <DocsSection id="anatomy" title={t("sectionAnatomy")}>
        ...
        <BreakdownBarDiagram
          segments={[
            { label: t("anatomy.labelChunk1"), value: 180, colorClass: "fill-blue-600 dark:fill-blue-400" },
            { label: t("anatomy.labelChunk2"), value: 195, colorClass: "fill-emerald-500 dark:fill-emerald-400" },
            { label: t("anatomy.labelChunk3"), value: 42, colorClass: "fill-orange-400 dark:fill-orange-500" },
          ]}
