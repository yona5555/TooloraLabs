...
export const RELATED_TOOLS = ["due-date-calculator", "pregnancy-calculator", "date-calculator", "age-calculator", "countdown-to-event-calculator"];

export async function getOvulationCalculatorTocItems(): Promise<TocItem[]> {
  const t = await getTranslations("docs.tools.ovulation-calculator");
  return [
...
export default async function OvulationCalculatorDocsPage() {
  const t = await getTranslations("docs.tools.ovulation-calculator");
  ...
  return (
    <>
      <DocsBreadcrumb items={[{ label: tNav("overview"), href: "/docs" }, { label: tNav("toolsGuide"), href: "/docs" }, { label: tTools("ovulation-calculator.title") }]} />
      <DocsHero title={tTools("ovulation-calculator.title")} version={t("version")} description={t("description")} />
      ...
      <DocsSection id="anatomy" title={t("sectionAnatomy")}>
        ...
        <DotPlotDiagram
          values={[9, 14, 15, 28]}
          markers={[
            { value: 9, label: t("anatomy.labelFertileStart"), colorClass: "stroke-emerald-500 dark:stroke-emerald-400 fill-emerald-500 dark:fill-emerald-400" },
            { value: 14, label: t("anatomy.labelOvulation"), colorClass: "stroke-blue-600 dark:stroke-blue-400 fill-blue-600 dark:fill-blue-400" },
            { value: 28, label: t("anatomy.labelNextPeriod"), colorClass: "stroke-purple-500 dark:stroke-purple-400 fill-purple-500 dark:fill-purple-400" },
          ]}
