...
export const RELATED_TOOLS = ["kinematics-calculator", "projectile-motion-calculator", "energy-work-power-calculator"];

export async function getForceCalculatorTocItems(): Promise<TocItem[]> {
  const t = await getTranslations("docs.tools.force-calculator");
  return [
...
export default async function ForceCalculatorDocsPage() {
  const t = await getTranslations("docs.tools.force-calculator");
  ...
  return (
    <>
      <DocsBreadcrumb items={[{ label: tNav("overview"), href: "/docs" }, { label: tNav("toolsGuide"), href: "/docs" }, { label: tTools("force-calculator.title") }]} />
      <DocsHero title={tTools("force-calculator.title")} version={t("version")} description={t("description")} />
