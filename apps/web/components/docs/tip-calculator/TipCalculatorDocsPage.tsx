...
export const RELATED_TOOLS = ["discount-calculator", "sales-tax-calculator", "invoice-generator", "batch-invoice-calculator", "percentage-calculator"];

export async function getTipCalculatorTocItems(): Promise<TocItem[]> {
  const t = await getTranslations("docs.tools.tip-calculator");
  return [
...
export default async function TipCalculatorDocsPage() {
  const t = await getTranslations("docs.tools.tip-calculator");
  ...
  return (
    <>
      <DocsBreadcrumb items={[{ label: tNav("overview"), href: "/docs" }, { label: tNav("toolsGuide"), href: "/docs" }, { label: tTools("tip-calculator.title") }]} />
      <DocsHero title={tTools("tip-calculator.title")} version={t("version")} description={t("description")} />
      ...
      <DocsSection id="anatomy" title={t("sectionAnatomy")}>
        ...
        <BreakdownBarDiagram
          segments={[
            { label: t("anatomy.labelBill"), value: 80, colorClass: "fill-blue-600 dark:fill-blue-400" },
            { label: t("anatomy.labelTip"), value: 16, colorClass: "fill-emerald-500 dark:fill-emerald-400" },
          ]}
