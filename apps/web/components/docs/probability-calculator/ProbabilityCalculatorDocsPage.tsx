...
import VennOverlapDiagram from "@/components/docs/VennOverlapDiagram";
import ValueComparisonBarDiagram from "@/components/docs/ValueComparisonBarDiagram";

export const RELATED_TOOLS = ["statistics-calculator", "random-number-generator", "mean-median-mode-range-calculator", "standard-deviation-calculator", "gcf-lcm-calculator"];

export async function getProbabilityCalculatorTocItems(): Promise<TocItem[]> {
  const t = await getTranslations("docs.tools.probability-calculator");
  return [
    { id: "formula", label: t("sectionFormula") },
    { id: "flowchart", label: t("sectionFlowchart") },
    { id: "venn", label: t("sectionVenn") },
...
export default async function ProbabilityCalculatorDocsPage() {
  const t = await getTranslations("docs.tools.probability-calculator");
  ...
  return (
    <>
      <DocsBreadcrumb items={[{ label: tNav("overview"), href: "/docs" }, { label: tNav("toolsGuide"), href: "/docs" }, { label: tTools("probability-calculator.title") }]} />
      <DocsHero title={tTools("probability-calculator.title")} version={t("version")} description={t("description")} />
      ...
      <DocsSection id="venn" title={t("sectionVenn")}>
        <p className="mb-4 text-zinc-600 dark:text-zinc-300">{t("venn.intro")}</p>
        <VennOverlapDiagram
          leftLabel={t("venn.leftLabel")}
          rightLabel={t("venn.rightLabel")}
          overlapLabel={t("venn.overlapLabel")}
          leftOnlyItems={t("venn.leftOnlyItems")}
          overlapItems={t("venn.overlapItems")}
          rightOnlyItems={t("venn.rightOnlyItems")}
          caption={t("venn.caption")}
        />
      </DocsSection>
      ...
      <DocsSection id="anatomy" title={t("sectionAnatomy")}>
        ...
        <ValueComparisonBarDiagram
          bars={[
            { label: t("anatomy.labelSingle"), value: 50, colorClass: "fill-blue-600 dark:fill-blue-400" },
            { label: t("anatomy.labelAnd"), value: 25, colorClass: "fill-purple-500 dark:fill-purple-400" },
            { label: t("anatomy.labelOr"), value: 75, colorClass: "fill-emerald-500 dark:fill-emerald-400" },
          ]}
