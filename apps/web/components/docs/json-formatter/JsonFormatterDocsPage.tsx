...
export const RELATED_TOOLS = ["base64-tool", "password-generator", "qr-code-generator"];

export async function getJsonFormatterTocItems(): Promise<TocItem[]> {
  const t = await getTranslations("docs.tools.json-formatter");
  return [
...
export default async function JsonFormatterDocsPage() {
  const t = await getTranslations("docs.tools.json-formatter");
  ...
  return (
    <>
      <DocsBreadcrumb items={[{ label: tNav("overview"), href: "/docs" }, { label: tNav("toolsGuide"), href: "/docs" }, { label: tTools("json-formatter.title") }]} />
      <DocsHero title={tTools("json-formatter.title")} version={t("version")} description={t("description")} />
      ...
      <DocsSection id="anatomy" title={t("sectionAnatomy")}>
        ...
        <ValueComparisonBarDiagram
          bars={[
            { label: t("anatomy.labelMinified"), value: 37, colorClass: "fill-blue-600 dark:fill-blue-400" },
            { label: t("anatomy.labelTwoSpace"), value: 50, colorClass: "fill-emerald-500 dark:fill-emerald-400" },
            { label: t("anatomy.labelFourSpace"), value: 56, colorClass: "fill-purple-500 dark:fill-purple-400" },
          ]}
