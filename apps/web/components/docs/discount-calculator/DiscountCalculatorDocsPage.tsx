...
export const RELATED_TOOLS = ["sales-tax-calculator", "tip-calculator", "invoice-generator", "break-even-calculator", "percentage-calculator"];

export async function getDiscountCalculatorTocItems(): Promise<TocItem[]> {
  const t = await getTranslations("docs.tools.discount-calculator");
  return [
...
export default async function DiscountCalculatorDocsPage() {
  const t = await getTranslations("docs.tools.discount-calculator");
  ...
  return (
    <>
      <DocsBreadcrumb items={[{ label: tNav("overview"), href: "/docs" }, { label: tNav("toolsGuide"), href: "/docs" }, { label: tTools("discount-calculator.title") }]} />
      <DocsHero title={tTools("discount-calculator.title")} version={t("version")} description={t("description")} />
      ...
      <DocsSection id="anatomy" title={t("sectionAnatomy")}>
        ...
        <BreakdownBarDiagram
          segments={[
            { label: t("anatomy.labelFinal"), value: 70, colorClass: "fill-blue-600 dark:fill-blue-400" },
            { label: t("anatomy.labelSaved"), value: 30, colorClass: "fill-emerald-500 dark:fill-emerald-400" },
          ]}
