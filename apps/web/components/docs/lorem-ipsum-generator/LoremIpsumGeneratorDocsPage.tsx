...
export const RELATED_TOOLS = ["text-logo-generator", "word-counter", "readability-score-calculator"];

export async function getLoremIpsumGeneratorTocItems(): Promise<TocItem[]> {
  const t = await getTranslations("docs.tools.lorem-ipsum-generator");
  return [
...
export default async function LoremIpsumGeneratorDocsPage() {
  const t = await getTranslations("docs.tools.lorem-ipsum-generator");
  ...
  return (
    <>
      <DocsBreadcrumb items={[{ label: tNav("overview"), href: "/docs" }, { label: tNav("toolsGuide"), href: "/docs" }, { label: tTools("lorem-ipsum-generator.title") }]} />
      <DocsHero title={tTools("lorem-ipsum-generator.title")} version={t("version")} description={t("description")} />
      ...
      <DocsSection id="anatomy" title={t("sectionAnatomy")}>
        ...
        <ValueComparisonBarDiagram
          bars={[
            { label: t("anatomy.labelClassic"), value: 62, colorClass: "fill-blue-600 dark:fill-blue-400" },
            { label: t("anatomy.labelModern"), value: 51, colorClass: "fill-emerald-500 dark:fill-emerald-400" },
          ]}
