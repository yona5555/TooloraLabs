...
export const RELATED_TOOLS = ["surface-area-calculator", "volume-calculator", "circle-calculator", "triangle-calculator", "step-by-step-math-solver"];

export async function getAreaCalculatorTocItems(): Promise<TocItem[]> {
  const t = await getTranslations("docs.tools.area-calculator");
  return [
...
export default async function AreaCalculatorDocsPage() {
  const t = await getTranslations("docs.tools.area-calculator");
  ...
  return (
    <>
      <DocsBreadcrumb items={[{ label: tNav("overview"), href: "/docs" }, { label: tNav("toolsGuide"), href: "/docs" }, { label: tTools("area-calculator.title") }]} />
      <DocsHero title={tTools("area-calculator.title")} version={t("version")} description={t("description")} />
      ...
      <DocsSection id="anatomy" title={t("sectionAnatomy")}>
        ...
        <ValueComparisonBarDiagram
          bars={[
            { label: t("anatomy.labelSquare"), value: 100, colorClass: "fill-blue-600 dark:fill-blue-400" },
            { label: t("anatomy.labelCircle"), value: 78.5, colorClass: "fill-emerald-500 dark:fill-emerald-400" },
            { label: t("anatomy.labelTriangle"), value: 50, colorClass: "fill-purple-500 dark:fill-purple-400" },
          ]}
