...
export const RELATED_TOOLS = ["world-time-converter", "unit-converter", "countdown-to-event-calculator"];

export async function getWeatherForecastTocItems(): Promise<TocItem[]> {
  const t = await getTranslations("docs.tools.weather-forecast");
  return [
...
export default async function WeatherForecastDocsPage() {
  const t = await getTranslations("docs.tools.weather-forecast");
  ...
  return (
    <>
      <DocsBreadcrumb items={[{ label: tNav("overview"), href: "/docs" }, { label: tNav("toolsGuide"), href: "/docs" }, { label: tTools("weather-forecast.title") }]} />
      <DocsHero title={tTools("weather-forecast.title")} version={t("version")} description={t("description")} />
      ...
      <DocsSection id="anatomy" title={t("sectionAnatomy")}>
        ...
        <ValueComparisonBarDiagram
          bars={[
            { label: t("anatomy.labelHigh"), value: 24, colorClass: "fill-orange-400 dark:fill-orange-500" },
            { label: t("anatomy.labelLow"), value: 16, colorClass: "fill-blue-500 dark:fill-blue-400" },
          ]}
