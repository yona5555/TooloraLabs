...
export const RELATED_TOOLS = ["unit-converter", "file-size-converter", "zip-compressor"];

export async function getImageConverterTocItems(): Promise<TocItem[]> {
  const t = await getTranslations("docs.tools.image-converter");
  return [
...
export default async function ImageConverterDocsPage() {
  const t = await getTranslations("docs.tools.image-converter");
  ...
  return (
    <>
      <DocsBreadcrumb items={[{ label: tNav("overview"), href: "/docs" }, { label: tNav("toolsGuide"), href: "/docs" }, { label: tTools("image-converter.title") }]} />
      <DocsHero title={tTools("image-converter.title")} version={t("version")} description={t("description")} />
      ...
      <DocsSection id="anatomy" title={t("sectionAnatomy")}>
        ...
        <ValueComparisonBarDiagram
          bars={[
            { label: t("anatomy.labelOriginal"), value: 2400, colorClass: "fill-zinc-400 dark:fill-zinc-500" },
            { label: t("anatomy.labelConverted"), value: 640, colorClass: "fill-blue-600 dark:fill-blue-400" },
          ]}
