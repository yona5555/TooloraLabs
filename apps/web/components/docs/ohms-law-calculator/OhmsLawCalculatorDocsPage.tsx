...
export const RELATED_TOOLS = ["energy-work-power-calculator", "unit-converter", "scientific-calculator"];

export async function getOhmsLawCalculatorTocItems(): Promise<TocItem[]> {
  const t = await getTranslations("docs.tools.ohms-law-calculator");
  return [
...
export default async function OhmsLawCalculatorDocsPage() {
  const t = await getTranslations("docs.tools.ohms-law-calculator");
  ...
  return (
    <>
      <DocsBreadcrumb items={[{ label: tNav("overview"), href: "/docs" }, { label: tNav("toolsGuide"), href: "/docs" }, { label: tTools("ohms-law-calculator.title") }]} />
      <DocsHero title={tTools("ohms-law-calculator.title")} version={t("version")} description={t("description")} />
