...
import NumberLineMagnitudeDiagram from "@/components/docs/NumberLineMagnitudeDiagram";

export const RELATED_TOOLS = ["bmi-calculator", "ideal-weight-calculator", "bmr-calculator", "tdee-calculator", "macro-calculator"];

export async function getBodyFatCalculatorTocItems(): Promise<TocItem[]> {
  const t = await getTranslations("docs.tools.body-fat-calculator");
  return [
...
export default async function BodyFatCalculatorDocsPage() {
  const t = await getTranslations("docs.tools.body-fat-calculator");
  ...
  return (
    <>
      <DocsBreadcrumb items={[{ label: tNav("overview"), href: "/docs" }, { label: tNav("toolsGuide"), href: "/docs" }, { label: tTools("body-fat-calculator.title") }]} />
      <DocsHero title={tTools("body-fat-calculator.title")} version={t("version")} description={t("description")} />
      ...
      <DocsSection id="anatomy" title={t("sectionAnatomy")}>
        ...
        <NumberLineMagnitudeDiagram
          markers={[
            { label: t("anatomy.labelEssential"), exponent: 5 },
            { label: t("anatomy.labelAthletes"), exponent: 13 },
            { label: t("anatomy.labelFitness"), exponent: 17 },
            { label: t("anatomy.labelAverage"), exponent: 24 },
          ]}
          highlightExponent={15}
