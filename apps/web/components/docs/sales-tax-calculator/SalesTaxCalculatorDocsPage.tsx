...
export const RELATED_TOOLS = ["discount-calculator", "tip-calculator", "invoice-generator", "batch-invoice-calculator", "percentage-calculator"];

export async function getSalesTaxCalculatorTocItems(): Promise<TocItem[]> {
  const t = await getTranslations("docs.tools.sales-tax-calculator");
  return [
...
export default async function SalesTaxCalculatorDocsPage() {
  const t = await getTranslations("docs.tools.sales-tax-calculator");
  ...
  return (
    <>
      <DocsBreadcrumb items={[{ label: tNav("overview"), href: "/docs" }, { label: tNav("toolsGuide"), href: "/docs" }, { label: tTools("sales-tax-calculator.title") }]} />
      <DocsHero title={tTools("sales-tax-calculator.title")} version={t("version")} description={t("description")} />
      ...
      <DocsSection id="anatomy" title={t("sectionAnatomy")}>
        ...
        <BreakdownBarDiagram
          segments={[
            { label: t("anatomy.labelPrice"), value: 100, colorClass: "fill-blue-600 dark:fill-blue-400" },
            { label: t("anatomy.labelTax"), value: 8.5, colorClass: "fill-amber-500 dark:fill-amber-400" },
          ]}
