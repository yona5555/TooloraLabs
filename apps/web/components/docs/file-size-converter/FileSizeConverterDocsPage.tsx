...
export const RELATED_TOOLS = ["zip-compressor", "pdf-merge-split", "unit-converter"];

export async function getFileSizeConverterTocItems(): Promise<TocItem[]> {
  const t = await getTranslations("docs.tools.file-size-converter");
  return [
...
export default async function FileSizeConverterDocsPage() {
  const t = await getTranslations("docs.tools.file-size-converter");
  ...
  return (
    <>
      <DocsBreadcrumb items={[{ label: tNav("overview"), href: "/docs" }, { label: tNav("toolsGuide"), href: "/docs" }, { label: tTools("file-size-converter.title") }]} />
      <DocsHero title={tTools("file-size-converter.title")} version={t("version")} description={t("description")} />
      ...
      <DocsSection id="anatomy" title={t("sectionAnatomy")}>
        ...
        <ValueComparisonBarDiagram
          bars={[
            { label: t("anatomy.labelDecimal"), value: 1.0, colorClass: "fill-blue-600 dark:fill-blue-400" },
            { label: t("anatomy.labelBinary"), value: 0.931, colorClass: "fill-emerald-500 dark:fill-emerald-400" },
          ]}
