...
export const RELATED_TOOLS = ["molarity-calculator", "unit-converter", "density-calculator"];

export async function getIdealGasLawCalculatorTocItems(): Promise<TocItem[]> {
  const t = await getTranslations("docs.tools.ideal-gas-law-calculator");
  return [
...
export default async function IdealGasLawCalculatorDocsPage() {
  const t = await getTranslations("docs.tools.ideal-gas-law-calculator");
  ...
  return (
    <>
      <DocsBreadcrumb items={[{ label: tNav("overview"), href: "/docs" }, { label: tNav("toolsGuide"), href: "/docs" }, { label: tTools("ideal-gas-law-calculator.title") }]} />
      <DocsHero title={tTools("ideal-gas-law-calculator.title")} version={t("version")} description={t("description")} />
