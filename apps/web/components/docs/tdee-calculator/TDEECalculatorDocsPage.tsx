...
export const RELATED_TOOLS = ["bmr-calculator", "macro-calculator", "bmi-calculator", "ideal-weight-calculator", "target-heart-rate-calculator"];

export async function getTDEECalculatorTocItems(): Promise<TocItem[]> {
  const t = await getTranslations("docs.tools.tdee-calculator");
  return [
...
export default async function TDEECalculatorDocsPage() {
  const t = await getTranslations("docs.tools.tdee-calculator");
  ...
  return (
    <>
      <DocsBreadcrumb items={[{ label: tNav("overview"), href: "/docs" }, { label: tNav("toolsGuide"), href: "/docs" }, { label: tTools("tdee-calculator.title") }]} />
      <DocsHero title={tTools("tdee-calculator.title")} version={t("version")} description={t("description")} />
      ...
      <DocsSection id="anatomy" title={t("sectionAnatomy")}>
        ...
        <ValueComparisonBarDiagram
          bars={[
            { label: t("anatomy.labelSedentary"), value: 1800, colorClass: "fill-blue-600 dark:fill-blue-400" },
            { label: t("anatomy.labelModerate"), value: 2325, colorClass: "fill-emerald-500 dark:fill-emerald-400" },
            { label: t("anatomy.labelVeryActive"), value: 2850, colorClass: "fill-purple-500 dark:fill-purple-400" },
          ]}
