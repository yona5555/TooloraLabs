...
export const RELATED_TOOLS = ["due-date-calculator", "ovulation-calculator", "date-calculator", "age-calculator", "countdown-to-event-calculator"];

export async function getPregnancyCalculatorTocItems(): Promise<TocItem[]> {
  const t = await getTranslations("docs.tools.pregnancy-calculator");
  return [
...
export default async function PregnancyCalculatorDocsPage() {
  const t = await getTranslations("docs.tools.pregnancy-calculator");
  ...
  return (
    <>
      <DocsBreadcrumb items={[{ label: tNav("overview"), href: "/docs" }, { label: tNav("toolsGuide"), href: "/docs" }, { label: tTools("pregnancy-calculator.title") }]} />
      <DocsHero title={tTools("pregnancy-calculator.title")} version={t("version")} description={t("description")} />
      ...
      <DocsSection id="anatomy" title={t("sectionAnatomy")}>
        ...
        <BreakdownBarDiagram
          segments={[
            { label: t("anatomy.labelTrimester1"), value: 13, colorClass: "fill-blue-600 dark:fill-blue-400" },
            { label: t("anatomy.labelTrimester2"), value: 14, colorClass: "fill-emerald-500 dark:fill-emerald-400" },
            { label: t("anatomy.labelTrimester3"), value: 13, colorClass: "fill-purple-500 dark:fill-purple-400" },
          ]}
