...
export const RELATED_TOOLS = ["file-name-sanitizer", "csv-json-converter", "pdf-merge-split"];

export async function getDuplicateLineRemoverTocItems(): Promise<TocItem[]> {
  const t = await getTranslations("docs.tools.duplicate-line-remover");
  return [
...
export default async function DuplicateLineRemoverDocsPage() {
  const t = await getTranslations("docs.tools.duplicate-line-remover");
  ...
  return (
    <>
      <DocsBreadcrumb items={[{ label: tNav("overview"), href: "/docs" }, { label: tNav("toolsGuide"), href: "/docs" }, { label: tTools("duplicate-line-remover.title") }]} />
      <DocsHero title={tTools("duplicate-line-remover.title")} version={t("version")} description={t("description")} />
      ...
      <DocsSection id="anatomy" title={t("sectionAnatomy")}>
        ...
        <BreakdownBarDiagram
          segments={[
            { label: t("anatomy.labelKept"), value: 7, colorClass: "fill-emerald-500 dark:fill-emerald-400" },
            { label: t("anatomy.labelRemoved"), value: 3, colorClass: "fill-orange-500 dark:fill-orange-400" },
          ]}
