...
export const RELATED_TOOLS = ["tdee-calculator", "macro-calculator", "bmi-calculator", "ideal-weight-calculator", "body-fat-calculator"];

export async function getBMRCalculatorTocItems(): Promise<TocItem[]> {
  const t = await getTranslations("docs.tools.bmr-calculator");
  return [
...
export default async function BMRCalculatorDocsPage() {
  const t = await getTranslations("docs.tools.bmr-calculator");
  ...
  return (
    <>
      <DocsBreadcrumb items={[{ label: tNav("overview"), href: "/docs" }, { label: tNav("toolsGuide"), href: "/docs" }, { label: tTools("bmr-calculator.title") }]} />
      <DocsHero title={tTools("bmr-calculator.title")} version={t("version")} description={t("description")} />
      ...
      <DocsSection id="anatomy" title={t("sectionAnatomy")}>
        ...
        <ValueComparisonBarDiagram
          bars={[
            { label: t("anatomy.labelHarrisBenedict"), value: 1865, colorClass: "fill-blue-600 dark:fill-blue-400" },
            { label: t("anatomy.labelMifflinStJeor"), value: 1780, colorClass: "fill-emerald-500 dark:fill-emerald-400" },
          ]}
