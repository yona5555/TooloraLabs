...
export const RELATED_TOOLS = ["tdee-calculator", "bmr-calculator", "bmi-calculator", "ideal-weight-calculator", "body-fat-calculator"];

export async function getMacroCalculatorTocItems(): Promise<TocItem[]> {
  const t = await getTranslations("docs.tools.macro-calculator");
  return [
...
export default async function MacroCalculatorDocsPage() {
  const t = await getTranslations("docs.tools.macro-calculator");
  ...
  return (
    <>
      <DocsBreadcrumb items={[{ label: tNav("overview"), href: "/docs" }, { label: tNav("toolsGuide"), href: "/docs" }, { label: tTools("macro-calculator.title") }]} />
      <DocsHero title={tTools("macro-calculator.title")} version={t("version")} description={t("description")} />
      ...
      <DocsSection id="anatomy" title={t("sectionAnatomy")}>
        ...
        <BreakdownBarDiagram
          segments={[
            { label: t("anatomy.labelProtein"), value: 400, colorClass: "fill-blue-600 dark:fill-blue-400" },
            { label: t("anatomy.labelCarbs"), value: 1000, colorClass: "fill-emerald-500 dark:fill-emerald-400" },
            { label: t("anatomy.labelFat"), value: 600, colorClass: "fill-amber-500 dark:fill-amber-400" },
          ]}
