...
export const RELATED_TOOLS = ["text-to-speech", "spelling-grammar-checker", "word-counter"];

export async function getSpeechToTextTocItems(): Promise<TocItem[]> {
  const t = await getTranslations("docs.tools.speech-to-text");
  return [
...
export default async function SpeechToTextDocsPage() {
  const t = await getTranslations("docs.tools.speech-to-text");
  ...
  return (
    <>
      <DocsBreadcrumb items={[{ label: tNav("overview"), href: "/docs" }, { label: tNav("toolsGuide"), href: "/docs" }, { label: tTools("speech-to-text.title") }]} />
      <DocsHero title={tTools("speech-to-text.title")} version={t("version")} description={t("description")} />
      ...
      <DocsSection id="anatomy" title={t("sectionAnatomy")}>
        ...
        <BreakdownBarDiagram
          segments={[
            { label: t("anatomy.labelSegment1"), value: 11, colorClass: "fill-blue-600 dark:fill-blue-400" },
            { label: t("anatomy.labelSegment2"), value: 11, colorClass: "fill-emerald-500 dark:fill-emerald-400" },
            { label: t("anatomy.labelSegment3"), value: 5, colorClass: "fill-orange-400 dark:fill-orange-500" },
          ]}
