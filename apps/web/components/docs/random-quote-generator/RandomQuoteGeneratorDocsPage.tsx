...
export const RELATED_TOOLS = ["love-calculator", "dice-roller", "citation-generator"];

export async function getRandomQuoteGeneratorTocItems(): Promise<TocItem[]> {
  const t = await getTranslations("docs.tools.random-quote-generator");
  return [
    { id: "formula", label: t("sectionFormula") },
    { id: "flowchart", label: t("sectionFlowchart") },
    { id: "pools", label: t("sectionPools") },
...
export default async function RandomQuoteGeneratorDocsPage() {
  const t = await getTranslations("docs.tools.random-quote-generator");
  ...
  return (
    <>
      <DocsBreadcrumb items={[{ label: tNav("overview"), href: "/docs" }, { label: tNav("toolsGuide"), href: "/docs" }, { label: tTools("random-quote-generator.title") }]} />
      <DocsHero title={tTools("random-quote-generator.title")} version={t("version")} description={t("description")} />
      ...
      <DocsSection id="pools" title={t("sectionPools")}>
        <p className="mb-4 text-zinc-600 dark:text-zinc-300">{t("pools.intro")}</p>
        <ValueComparisonBarDiagram
          bars={[
            { label: t("pools.labelMotivational"), value: 10, colorClass: "fill-blue-600 dark:fill-blue-400" },
            { label: t("pools.labelLiterary"), value: 10, colorClass: "fill-emerald-500 dark:fill-emerald-400" },
            { label: t("pools.labelPhilosophical"), value: 10, colorClass: "fill-purple-500 dark:fill-purple-400" },
          ]}
          caption={t("pools.caption")}
        />
      </DocsSection>
      ...
      <DocsSection id="anatomy" title={t("sectionAnatomy")}>
        ...
        <QuoteAnatomyDiagram
          fields={[
            { label: t("anatomy.labelText"), value: t("anatomy.valueText"), colorClass: "bg-blue-600 dark:bg-blue-400" },
            { label: t("anatomy.labelAuthor"), value: t("anatomy.valueAuthor"), colorClass: "bg-emerald-500 dark:bg-emerald-400" },
            { label: t("anatomy.labelSource"), value: t("anatomy.valueSource"), colorClass: "bg-purple-500 dark:bg-purple-400" },
            { label: t("anatomy.labelCategory"), value: t("anatomy.valueCategory"), colorClass: "bg-orange-400 dark:bg-orange-500" },
          ]}
