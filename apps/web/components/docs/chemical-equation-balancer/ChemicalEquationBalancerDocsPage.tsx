...
export const RELATED_TOOLS = ["stoichiometry-calculator", "molar-mass-calculator", "molarity-calculator"];

export async function getChemicalEquationBalancerTocItems(): Promise<TocItem[]> {
  const t = await getTranslations("docs.tools.chemical-equation-balancer");
  return [
...
export default async function ChemicalEquationBalancerDocsPage() {
  const t = await getTranslations("docs.tools.chemical-equation-balancer");
  ...
  return (
    <>
      <DocsBreadcrumb items={[{ label: tNav("overview"), href: "/docs" }, { label: tNav("toolsGuide"), href: "/docs" }, { label: tTools("chemical-equation-balancer.title") }]} />
      <DocsHero title={tTools("chemical-equation-balancer.title")} version={t("version")} description={t("description")} />
      ...
      <DocsSection id="anatomy" title={t("sectionAnatomy")}>
        ...
        <BreakdownBarDiagram
          segments={[
            { label: t("anatomy.labelC"), value: 1, colorClass: "fill-blue-600 dark:fill-blue-400" },
            { label: t("anatomy.labelH"), value: 4, colorClass: "fill-emerald-500 dark:fill-emerald-400" },
            { label: t("anatomy.labelO"), value: 4, colorClass: "fill-orange-500 dark:fill-orange-400" },
          ]}
          totalLabel={t("anatomy.reactantsLabel")}
          caption={t("anatomy.reactantsCaption")}
        />
        <div className="mt-6">
          <BreakdownBarDiagram
            segments={[
            ...
            totalLabel={t("anatomy.productsLabel")}
