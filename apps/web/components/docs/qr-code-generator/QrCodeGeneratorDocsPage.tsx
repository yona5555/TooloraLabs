...
export const RELATED_TOOLS = ["barcode-generator", "color-palette-generator", "base64-tool"];

export async function getQrCodeGeneratorTocItems(): Promise<TocItem[]> {
  const t = await getTranslations("docs.tools.qr-code-generator");
  return [
...
export default async function QrCodeGeneratorDocsPage() {
  const t = await getTranslations("docs.tools.qr-code-generator");
  ...
  return (
    <>
      <DocsBreadcrumb items={[{ label: tNav("overview"), href: "/docs" }, { label: tNav("toolsGuide"), href: "/docs" }, { label: tTools("qr-code-generator.title") }]} />
      <DocsHero title={tTools("qr-code-generator.title")} version={t("version")} description={t("description")} />
      ...
      <DocsSection id="anatomy" title={t("sectionAnatomy")}>
        ...
        <ValueComparisonBarDiagram
          bars={[
            { label: t("anatomy.labelL"), value: 7, colorClass: "fill-blue-600 dark:fill-blue-400" },
            { label: t("anatomy.labelM"), value: 15, colorClass: "fill-emerald-500 dark:fill-emerald-400" },
            { label: t("anatomy.labelQ"), value: 25, colorClass: "fill-amber-500 dark:fill-amber-400" },
            { label: t("anatomy.labelH"), value: 30, colorClass: "fill-purple-500 dark:fill-purple-400" },
          ]}
