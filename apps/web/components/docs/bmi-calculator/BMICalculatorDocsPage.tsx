...
export const RELATED_TOOLS = ["ideal-weight-calculator", "body-fat-calculator", "bmr-calculator", "tdee-calculator", "macro-calculator"];

export async function getBMICalculatorTocItems(): Promise<TocItem[]> {
  const t = await getTranslations("docs.tools.bmi-calculator");
  return [
...
export default async function BMICalculatorDocsPage() {
  const t = await getTranslations("docs.tools.bmi-calculator");
  ...
  return (
    <>
      <DocsBreadcrumb items={[{ label: tNav("overview"), href: "/docs" }, { label: tNav("toolsGuide"), href: "/docs" }, { label: tTools("bmi-calculator.title") }]} />
      <DocsHero title={tTools("bmi-calculator.title")} version={t("version")} description={t("description")} />
      ...
      <DocsSection id="anatomy" title={t("sectionAnatomy")}>
        ...
        <NumberLineMagnitudeDiagram
          markers={[
            { label: t("anatomy.labelUnderweight"), exponent: 16 },
            { label: t("anatomy.labelNormal"), exponent: 18.5 },
            { label: t("anatomy.labelOverweight"), exponent: 25 },
            { label: t("anatomy.labelObesity"), exponent: 30 },
          ]}
          highlightExponent={22}
