...
export const RELATED_TOOLS = ["molar-mass-calculator", "stoichiometry-calculator", "ph-calculator"];

export async function getMolarityCalculatorTocItems(): Promise<TocItem[]> {
  const t = await getTranslations("docs.tools.molarity-calculator");
  return [
...
export default async function MolarityCalculatorDocsPage() {
  const t = await getTranslations("docs.tools.molarity-calculator");
  ...
  return (
    <>
      <DocsBreadcrumb items={[{ label: tNav("overview"), href: "/docs" }, { label: tNav("toolsGuide"), href: "/docs" }, { label: tTools("molarity-calculator.title") }]} />
      <DocsHero title={tTools("molarity-calculator.title")} version={t("version")} description={t("description")} />
