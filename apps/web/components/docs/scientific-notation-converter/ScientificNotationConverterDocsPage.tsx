...
export const RELATED_TOOLS = ["significant-figures-calculator", "fraction-calculator", "percentage-calculator", "scientific-calculator", "standard-deviation-calculator"];

export async function getScientificNotationConverterTocItems(): Promise<TocItem[]> {
  const t = await getTranslations("docs.tools.scientific-notation-converter");
  return [
    { id: "formula", label: t("sectionFormula") },
    { id: "flowchart", label: t("sectionFlowchart") },
    { id: "magnitude", label: t("sectionMagnitude") },
...
export default async function ScientificNotationConverterDocsPage() {
  const t = await getTranslations("docs.tools.scientific-notation-converter");
  ...
  const magnitudeMarkers = t.raw("magnitude.markers") as { label: string; exponent: number }[];

  return (
    <>
      <DocsBreadcrumb items={[{ label: tNav("overview"), href: "/docs" }, { label: tNav("toolsGuide"), href: "/docs" }, { label: tTools("scientific-notation-converter.title") }]} />
      <DocsHero title={tTools("scientific-notation-converter.title")} version={t("version")} description={t("description")} />
      ...
      <DocsSection id="magnitude" title={t("sectionMagnitude")}>
        <p className="mb-4 text-zinc-600 dark:text-zinc-300">{t("magnitude.intro")}</p>
        <NumberLineMagnitudeDiagram markers={magnitudeMarkers} highlightExponent={9} caption={t("magnitude.caption")} />
      </DocsSection>
      ...
      <DocsSection id="anatomy" title={t("sectionAnatomy")}>
        ...
        <ValueComparisonBarDiagram
          bars={[
            { label: t("anatomy.labelCoefficient"), value: 4.2, colorClass: "fill-blue-600 dark:fill-blue-400" },
            { label: t("anatomy.labelExponent"), value: 9, colorClass: "fill-purple-500 dark:fill-purple-400" },
          ]}
