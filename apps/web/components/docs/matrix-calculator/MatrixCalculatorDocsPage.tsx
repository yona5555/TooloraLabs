...
import MatrixAnatomyDiagram from "./MatrixAnatomyDiagram";

export const RELATED_TOOLS = ["vector-calculator", "scientific-calculator", "step-by-step-math-solver", "graphing-calculator", "percentage-calculator"];

export async function getMatrixCalculatorTocItems(): Promise<TocItem[]> {
  const t = await getTranslations("docs.tools.matrix-calculator");
  return [
...
export default async function MatrixCalculatorDocsPage() {
  const t = await getTranslations("docs.tools.matrix-calculator");
  ...
  return (
    <>
      <DocsBreadcrumb items={[{ label: tNav("overview"), href: "/docs" }, { label: tNav("toolsGuide"), href: "/docs" }, { label: tTools("matrix-calculator.title") }]} />
      <DocsHero title={tTools("matrix-calculator.title")} version={t("version")} description={t("description")} />
      ...
      <DocsSection id="anatomy" title={t("sectionAnatomy")}>
        ...
        <MatrixAnatomyDiagram
          ...
          inv21={-0.2}
          inv22={0.4}
          labelMatrix={t("anatomy.labelMatrix")}
          labelDeterminant={t("anatomy.labelDeterminant")}
          labelInverse={t("anatomy.labelInverse")}
