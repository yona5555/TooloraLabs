...
export const RELATED_TOOLS = ["word-counter", "reading-time-calculator", "lorem-ipsum-generator"];

export async function getReadabilityScoreCalculatorTocItems(): Promise<TocItem[]> {
  const t = await getTranslations("docs.tools.readability-score-calculator");
  return [
...
export default async function ReadabilityScoreCalculatorDocsPage() {
  const t = await getTranslations("docs.tools.readability-score-calculator");
  ...
  return (
    <>
      <DocsBreadcrumb items={[{ label: tNav("overview"), href: "/docs" }, { label: tNav("toolsGuide"), href: "/docs" }, { label: tTools("readability-score-calculator.title") }]} />
      <DocsHero title={tTools("readability-score-calculator.title")} version={t("version")} description={t("description")} />
      ...
      <DocsSection id="anatomy" title={t("sectionAnatomy")}>
        ...
        <NumberLineMagnitudeDiagram
          markers={[
            { label: "30", exponent: 30 },
            { label: "60", exponent: 60 },
            { label: "90", exponent: 90 },
          ]}
          highlightExponent={65}
