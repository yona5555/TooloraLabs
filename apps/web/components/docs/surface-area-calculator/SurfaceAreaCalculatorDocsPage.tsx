...
export const RELATED_TOOLS = ["area-calculator", "volume-calculator", "circle-calculator", "triangle-calculator", "step-by-step-math-solver"];

export async function getSurfaceAreaCalculatorTocItems(): Promise<TocItem[]> {
  const t = await getTranslations("docs.tools.surface-area-calculator");
  return [
...
export default async function SurfaceAreaCalculatorDocsPage() {
  const t = await getTranslations("docs.tools.surface-area-calculator");
  ...
  return (
    <>
      <DocsBreadcrumb items={[{ label: tNav("overview"), href: "/docs" }, { label: tNav("toolsGuide"), href: "/docs" }, { label: tTools("surface-area-calculator.title") }]} />
      <DocsHero title={tTools("surface-area-calculator.title")} version={t("version")} description={t("description")} />
      ...
      <DocsSection id="anatomy" title={t("sectionAnatomy")}>
        ...
        <ValueComparisonBarDiagram
          bars={[
            { label: t("anatomy.labelCube"), value: 600, colorClass: "fill-blue-600 dark:fill-blue-400" },
            { label: t("anatomy.labelSphere"), value: 1257, colorClass: "fill-emerald-500 dark:fill-emerald-400" },
            { label: t("anatomy.labelCylinder"), value: 942, colorClass: "fill-purple-500 dark:fill-purple-400" },
          ]}
