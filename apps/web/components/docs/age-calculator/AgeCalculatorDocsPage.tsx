...
export const RELATED_TOOLS = ["date-calculator", "countdown-to-event-calculator", "due-date-calculator"];

export async function getAgeCalculatorTocItems(): Promise<TocItem[]> {
  const t = await getTranslations("docs.tools.age-calculator");
  return [
...
export default async function AgeCalculatorDocsPage() {
  const t = await getTranslations("docs.tools.age-calculator");
  ...
  return (
    <>
      <DocsBreadcrumb items={[{ label: tNav("overview"), href: "/docs" }, { label: tNav("toolsGuide"), href: "/docs" }, { label: tTools("age-calculator.title") }]} />
      <DocsHero title={tTools("age-calculator.title")} version={t("version")} description={t("description")} />
      ...
      <DocsSection id="anatomy" title={t("sectionAnatomy")}>
        ...
        <BreakdownBarDiagram
          segments={[
            { label: t("anatomy.labelSince"), value: 270, colorClass: "fill-blue-600 dark:fill-blue-400" },
            { label: t("anatomy.labelUntil"), value: 95, colorClass: "fill-zinc-300 dark:fill-zinc-700" },
          ]}
