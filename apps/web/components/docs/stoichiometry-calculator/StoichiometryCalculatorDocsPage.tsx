...
export const RELATED_TOOLS = ["chemical-equation-balancer", "molar-mass-calculator", "molarity-calculator"];

export async function getStoichiometryCalculatorTocItems(): Promise<TocItem[]> {
  const t = await getTranslations("docs.tools.stoichiometry-calculator");
  return [
...
export default async function StoichiometryCalculatorDocsPage() {
  const t = await getTranslations("docs.tools.stoichiometry-calculator");
  ...
  return (
    <>
      <DocsBreadcrumb items={[{ label: tNav("overview"), href: "/docs" }, { label: tNav("toolsGuide"), href: "/docs" }, { label: tTools("stoichiometry-calculator.title") }]} />
      <DocsHero title={tTools("stoichiometry-calculator.title")} version={t("version")} description={t("description")} />
