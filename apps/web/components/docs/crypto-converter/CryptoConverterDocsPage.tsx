...
export const RELATED_TOOLS = ["forex-converter", "commodities-tracker", "percentage-calculator"];

export async function getCryptoConverterTocItems(): Promise<TocItem[]> {
  const t = await getTranslations("docs.tools.crypto-converter");
  return [
...
export default async function CryptoConverterDocsPage() {
  const t = await getTranslations("docs.tools.crypto-converter");
  ...
  return (
    <>
      <DocsBreadcrumb items={[{ label: tNav("overview"), href: "/docs" }, { label: tNav("toolsGuide"), href: "/docs" }, { label: tTools("crypto-converter.title") }]} />
      <DocsHero title={tTools("crypto-converter.title")} version={t("version")} description={t("description")} />
