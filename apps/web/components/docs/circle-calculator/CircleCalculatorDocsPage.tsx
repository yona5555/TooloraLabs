...
import CircleAnatomyDiagram from "./CircleAnatomyDiagram";

export const RELATED_TOOLS = ["area-calculator", "surface-area-calculator", "volume-calculator", "triangle-calculator", "scientific-calculator"];

export async function getCircleCalculatorTocItems(): Promise<TocItem[]> {
  const t = await getTranslations("docs.tools.circle-calculator");
  return [
...
export default async function CircleCalculatorDocsPage() {
  const t = await getTranslations("docs.tools.circle-calculator");
  ...
  return (
    <>
      <DocsBreadcrumb items={[{ label: tNav("overview"), href: "/docs" }, { label: tNav("toolsGuide"), href: "/docs" }, { label: tTools("circle-calculator.title") }]} />
      <DocsHero title={tTools("circle-calculator.title")} version={t("version")} description={t("description")} />
      ...
      <DocsSection id="anatomy" title={t("sectionAnatomy")}>
        ...
        <CircleAnatomyDiagram
          radiusLabel={t("anatomy.labelRadius")}
          diameterLabel={t("anatomy.labelDiameter")}
          circumferenceLabel={t("anatomy.labelCircumference")}
