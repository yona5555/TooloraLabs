...
export const RELATED_TOOLS = ["molarity-calculator", "ideal-gas-law-calculator", "scientific-calculator"];

export async function getPhCalculatorTocItems(): Promise<TocItem[]> {
  const t = await getTranslations("docs.tools.ph-calculator");
  return [
...
export default async function PhCalculatorDocsPage() {
  const t = await getTranslations("docs.tools.ph-calculator");
  ...
  return (
    <>
      <DocsBreadcrumb items={[{ label: tNav("overview"), href: "/docs" }, { label: tNav("toolsGuide"), href: "/docs" }, { label: tTools("ph-calculator.title") }]} />
      <DocsHero title={tTools("ph-calculator.title")} version={t("version")} description={t("description")} />
      ...
      <DocsSection id="anatomy" title={t("sectionAnatomy")}>
        ...
        <NumberLineMagnitudeDiagram
          markers={[
            { label: t("anatomy.labelAcidic"), exponent: 0 },
            { label: t("anatomy.labelNeutral"), exponent: 7 },
            { label: t("anatomy.labelBasic"), exponent: 14 },
          ]}
          highlightExponent={4}
