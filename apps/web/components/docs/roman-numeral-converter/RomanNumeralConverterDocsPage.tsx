...
export const RELATED_TOOLS = ["unit-converter", "scientific-notation-converter", "base64-tool"];

export async function getRomanNumeralConverterTocItems(): Promise<TocItem[]> {
  const t = await getTranslations("docs.tools.roman-numeral-converter");
  return [
...
export default async function RomanNumeralConverterDocsPage() {
  const t = await getTranslations("docs.tools.roman-numeral-converter");
  ...
  return (
    <>
      <DocsBreadcrumb items={[{ label: tNav("overview"), href: "/docs" }, { label: tNav("toolsGuide"), href: "/docs" }, { label: tTools("roman-numeral-converter.title") }]} />
      <DocsHero title={tTools("roman-numeral-converter.title")} version={t("version")} description={t("description")} />
      ...
      <DocsSection id="anatomy" title={t("sectionAnatomy")}>
        ...
        <BreakdownBarDiagram
          segments={[
            { label: "M", value: 1000, colorClass: "fill-blue-600 dark:fill-blue-400" },
            { label: "CM", value: 900, colorClass: "fill-emerald-500 dark:fill-emerald-400" },
            { label: "XC", value: 90, colorClass: "fill-orange-400 dark:fill-orange-500" },
            { label: "IV", value: 4, colorClass: "fill-purple-500 dark:fill-purple-400" },
          ]}
