...
export const RELATED_TOOLS = ["date-calculator", "age-calculator", "world-time-converter"];

export async function getCountdownToEventCalculatorTocItems(): Promise<TocItem[]> {
  const t = await getTranslations("docs.tools.countdown-to-event-calculator");
  return [
...
export default async function CountdownToEventCalculatorDocsPage() {
  const t = await getTranslations("docs.tools.countdown-to-event-calculator");
  ...
  return (
    <>
      <DocsBreadcrumb items={[{ label: tNav("overview"), href: "/docs" }, { label: tNav("toolsGuide"), href: "/docs" }, { label: tTools("countdown-to-event-calculator.title") }]} />
      <DocsHero title={tTools("countdown-to-event-calculator.title")} version={t("version")} description={t("description")} />
      ...
      <DocsSection id="anatomy" title={t("sectionAnatomy")}>
        ...
        <BreakdownBarDiagram
          segments={[
            { label: t("anatomy.labelDays"), value: 172800, colorClass: "fill-blue-600 dark:fill-blue-400" },
            { label: t("anatomy.labelHours"), value: 18000, colorClass: "fill-emerald-500 dark:fill-emerald-400" },
            { label: t("anatomy.labelMinutes"), value: 1800, colorClass: "fill-purple-500 dark:fill-purple-400" },
            { label: t("anatomy.labelSeconds"), value: 15, colorClass: "fill-amber-500 dark:fill-amber-400" },
          ]}
