...
export const RELATED_TOOLS = ["mean-median-mode-range-calculator", "standard-deviation-calculator", "probability-calculator", "random-number-generator", "percentage-calculator"];

export async function getStatisticsCalculatorTocItems(): Promise<TocItem[]> {
  const t = await getTranslations("docs.tools.statistics-calculator");
  return [
...
const SAMPLE_VALUES = [4, 8, 6, 5, 3, 8, 9, 6];

export default async function StatisticsCalculatorDocsPage() {
  const t = await getTranslations("docs.tools.statistics-calculator");
  ...
  return (
    <>
      <DocsBreadcrumb items={[{ label: tNav("overview"), href: "/docs" }, { label: tNav("toolsGuide"), href: "/docs" }, { label: tTools("statistics-calculator.title") }]} />
      <DocsHero title={tTools("statistics-calculator.title")} version={t("version")} description={t("description")} />
      ...
      <DocsSection id="dotplot" title={t("sectionDotplot")}>
        ...
        <DotPlotDiagram
          ...
          markers={[
            { value: 6.125, label: t("dotplot.labelMean"), colorClass: "stroke-blue-600 dark:stroke-blue-400 fill-blue-600 dark:fill-blue-400" },
            { value: 6, label: t("dotplot.labelMedian"), colorClass: "stroke-emerald-500 dark:stroke-emerald-400 fill-emerald-500 dark:fill-emerald-400" },
          ]}
      ...
      <DocsSection id="anatomy" title={t("sectionAnatomy")}>
        ...
        <ValueComparisonBarDiagram
          bars={[
            { label: t("anatomy.labelPopulation"), value: 3.36, colorClass: "fill-blue-600 dark:fill-blue-400" },
            { label: t("anatomy.labelSample"), value: 3.6, colorClass: "fill-purple-500 dark:fill-purple-400" },
          ]}
