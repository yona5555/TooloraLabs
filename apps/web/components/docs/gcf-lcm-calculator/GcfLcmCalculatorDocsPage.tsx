...
export const RELATED_TOOLS = ["fraction-calculator", "percentage-calculator", "probability-calculator", "standard-deviation-calculator", "scientific-calculator"];

export async function getGcfLcmCalculatorTocItems(): Promise<TocItem[]> {
  const t = await getTranslations("docs.tools.gcf-lcm-calculator");
  return [
...
export default async function GcfLcmCalculatorDocsPage() {
  const t = await getTranslations("docs.tools.gcf-lcm-calculator");
  ...
  return (
    <>
      <DocsBreadcrumb items={[{ label: tNav("overview"), href: "/docs" }, { label: tNav("toolsGuide"), href: "/docs" }, { label: tTools("gcf-lcm-calculator.title") }]} />
      <DocsHero title={tTools("gcf-lcm-calculator.title")} version={t("version")} description={t("description")} />
      ...
      <DocsSection id="anatomy" title={t("sectionAnatomy")}>
        ...
        <ValueComparisonBarDiagram
          bars={[
            { label: t("anatomy.labelGcf"), value: 6, colorClass: "fill-blue-600 dark:fill-blue-400" },
            { label: t("anatomy.labelLcm"), value: 36, colorClass: "fill-emerald-500 dark:fill-emerald-400" },
          ]}
