...
export const RELATED_TOOLS = ["target-heart-rate-calculator", "tdee-calculator", "sleep-calculator", "bmr-calculator", "macro-calculator"];

export async function getPaceCalculatorTocItems(): Promise<TocItem[]> {
  const t = await getTranslations("docs.tools.pace-calculator");
  return [
...
export default async function PaceCalculatorDocsPage() {
  const t = await getTranslations("docs.tools.pace-calculator");
  ...
  return (
    <>
      <DocsBreadcrumb items={[{ label: tNav("overview"), href: "/docs" }, { label: tNav("toolsGuide"), href: "/docs" }, { label: tTools("pace-calculator.title") }]} />
      <DocsHero title={tTools("pace-calculator.title")} version={t("version")} description={t("description")} />
      ...
      <DocsSection id="anatomy" title={t("sectionAnatomy")}>
        ...
        <ScalingCurveDiagram points={RIEGEL_POINTS} xLabel={t("anatomy.xLabel")} yLabel={t("anatomy.yLabel")} caption={t("anatomy.caption")} />
      </DocsSection>
