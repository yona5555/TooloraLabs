...
import FractionBarModelDiagram from "@/components/docs/FractionBarModelDiagram";

export const RELATED_TOOLS = ["gcf-lcm-calculator", "percentage-calculator", "significant-figures-calculator", "scientific-notation-converter", "scientific-calculator"];

export async function getFractionCalculatorTocItems(): Promise<TocItem[]> {
  const t = await getTranslations("docs.tools.fraction-calculator");
  return [
    { id: "formula", label: t("sectionFormula") },
    { id: "flowchart", label: t("sectionFlowchart") },
    { id: "bar-model", label: t("sectionBarModel") },
...
export default async function FractionCalculatorDocsPage() {
  const t = await getTranslations("docs.tools.fraction-calculator");
  ...
  return (
    <>
      <DocsBreadcrumb items={[{ label: tNav("overview"), href: "/docs" }, { label: tNav("toolsGuide"), href: "/docs" }, { label: tTools("fraction-calculator.title") }]} />
      <DocsHero title={tTools("fraction-calculator.title")} version={t("version")} description={t("description")} />
      ...
      <DocsSection id="bar-model" title={t("sectionBarModel")}>
        <p className="mb-4 text-zinc-600 dark:text-zinc-300">{t("barModel.intro")}</p>
        <FractionBarModelDiagram numerator={3} denominator={4} caption={t("barModel.caption")} />
      </DocsSection>

      <DocsSection id="anatomy" title={t("sectionAnatomy")}>
        <p className="mb-4 text-zinc-600 dark:text-zinc-300">{t("anatomy.intro")}</p>
        <FractionBarModelDiagram numerator={7} denominator={4} caption={t("anatomy.caption")} />
      </DocsSection>
