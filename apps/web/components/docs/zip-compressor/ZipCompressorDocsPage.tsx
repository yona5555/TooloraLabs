...
export const RELATED_TOOLS = ["pdf-merge-split", "file-size-converter", "duplicate-line-remover"];

export async function getZipCompressorTocItems(): Promise<TocItem[]> {
  const t = await getTranslations("docs.tools.zip-compressor");
  return [
...
export default async function ZipCompressorDocsPage() {
  const t = await getTranslations("docs.tools.zip-compressor");
  ...
  return (
    <>
      <DocsBreadcrumb items={[{ label: tNav("overview"), href: "/docs" }, { label: tNav("toolsGuide"), href: "/docs" }, { label: tTools("zip-compressor.title") }]} />
      <DocsHero title={tTools("zip-compressor.title")} version={t("version")} description={t("description")} />
      ...
      <DocsSection id="anatomy" title={t("sectionAnatomy")}>
        ...
        <ValueComparisonBarDiagram
          bars={[
            { label: t("anatomy.labelText"), value: 30, colorClass: "fill-emerald-500 dark:fill-emerald-400" },
            { label: t("anatomy.labelCompressed"), value: 98, colorClass: "fill-orange-500 dark:fill-orange-400" },
          ]}
