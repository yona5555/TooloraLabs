...
export const RELATED_TOOLS = ["class-schedule-builder", "gpa-calculator", "citation-generator"];

export async function getStudyTimeCalculatorTocItems(): Promise<TocItem[]> {
  const t = await getTranslations("docs.tools.study-time-calculator");
  return [
...
export default async function StudyTimeCalculatorDocsPage() {
  const t = await getTranslations("docs.tools.study-time-calculator");
  ...
  return (
    <>
      <DocsBreadcrumb items={[{ label: tNav("overview"), href: "/docs" }, { label: tNav("toolsGuide"), href: "/docs" }, { label: tTools("study-time-calculator.title") }]} />
      <DocsHero title={tTools("study-time-calculator.title")} version={t("version")} description={t("description")} />
      ...
      <DocsSection id="anatomy" title={t("sectionAnatomy")}>
        ...
        <BreakdownBarDiagram
          segments={[
            { label: t("anatomy.labelWork"), value: 75, colorClass: "fill-blue-600 dark:fill-blue-400" },
            { label: t("anatomy.labelBreaks"), value: 15, colorClass: "fill-emerald-500 dark:fill-emerald-400" },
            { label: t("anatomy.labelLeftover"), value: 10, colorClass: "fill-zinc-300 dark:fill-zinc-700" },
          ]}
