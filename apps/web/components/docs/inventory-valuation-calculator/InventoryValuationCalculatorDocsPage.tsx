...
export const RELATED_TOOLS = ["invoice-generator", "batch-invoice-calculator", "break-even-calculator", "sales-tax-calculator", "discount-calculator"];

export async function getInventoryValuationCalculatorTocItems(): Promise<TocItem[]> {
  const t = await getTranslations("docs.tools.inventory-valuation-calculator");
  return [
...
export default async function InventoryValuationCalculatorDocsPage() {
  const t = await getTranslations("docs.tools.inventory-valuation-calculator");
  ...
  return (
    <>
      <DocsBreadcrumb items={[{ label: tNav("overview"), href: "/docs" }, { label: tNav("toolsGuide"), href: "/docs" }, { label: tTools("inventory-valuation-calculator.title") }]} />
      <DocsHero title={tTools("inventory-valuation-calculator.title")} version={t("version")} description={t("description")} />
      ...
      <DocsSection id="anatomy" title={t("sectionAnatomy")}>
        ...
        <ValueComparisonBarDiagram
          bars={[
            { label: t("anatomy.labelFifo"), value: 640, colorClass: "fill-blue-600 dark:fill-blue-400" },
            { label: t("anatomy.labelLifo"), value: 400, colorClass: "fill-amber-500 dark:fill-amber-400" },
            { label: t("anatomy.labelWac"), value: 520, colorClass: "fill-emerald-500 dark:fill-emerald-400" },
          ]}
