...
export const RELATED_TOOLS = ["barcode-generator", "qr-code-generator", "image-converter"];

export async function getColorPaletteGeneratorTocItems(): Promise<TocItem[]> {
  const t = await getTranslations("docs.tools.color-palette-generator");
  return [
...
export default async function ColorPaletteGeneratorDocsPage() {
  const t = await getTranslations("docs.tools.color-palette-generator");
  ...
  return (
    <>
      <DocsBreadcrumb items={[{ label: tNav("overview"), href: "/docs" }, { label: tNav("toolsGuide"), href: "/docs" }, { label: tTools("color-palette-generator.title") }]} />
      <DocsHero title={tTools("color-palette-generator.title")} version={t("version")} description={t("description")} />
      ...
      <DocsSection id="anatomy" title={t("sectionAnatomy")}>
        ...
        <NumberLineMagnitudeDiagram
          markers={[
            { label: t("anatomy.labelBase"), exponent: 0 },
            { label: t("anatomy.labelSecond"), exponent: 120 },
            { label: t("anatomy.labelThird"), exponent: 240 },
          ]}
          highlightExponent={0}
