...
export const RELATED_TOOLS = ["lorem-ipsum-generator", "word-counter", "readability-score-calculator"];

export async function getTextLogoGeneratorTocItems(): Promise<TocItem[]> {
  const t = await getTranslations("docs.tools.text-logo-generator");
  return [
...
export default async function TextLogoGeneratorDocsPage() {
  const t = await getTranslations("docs.tools.text-logo-generator");
  ...
  return (
    <>
      <DocsBreadcrumb items={[{ label: tNav("overview"), href: "/docs" }, { label: tNav("toolsGuide"), href: "/docs" }, { label: tTools("text-logo-generator.title") }]} />
      <DocsHero title={tTools("text-logo-generator.title")} version={t("version")} description={t("description")} />
      ...
      <DocsSection id="anatomy" title={t("sectionAnatomy")}>
        ...
        <BreakdownBarDiagram
          segments={[
            { label: t("anatomy.labelStyled"), value: 10, colorClass: "fill-blue-600 dark:fill-blue-400" },
            { label: t("anatomy.labelUnchanged"), value: 1, colorClass: "fill-zinc-300 dark:fill-zinc-700" },
          ]}
