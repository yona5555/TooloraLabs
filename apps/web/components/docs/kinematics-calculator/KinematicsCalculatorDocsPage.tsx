...
export const RELATED_TOOLS = ["projectile-motion-calculator", "force-calculator", "energy-work-power-calculator"];

export async function getKinematicsCalculatorTocItems(): Promise<TocItem[]> {
  const t = await getTranslations("docs.tools.kinematics-calculator");
  return [
...
export default async function KinematicsCalculatorDocsPage() {
  const t = await getTranslations("docs.tools.kinematics-calculator");
  ...
  return (
    <>
      <DocsBreadcrumb items={[{ label: tNav("overview"), href: "/docs" }, { label: tNav("toolsGuide"), href: "/docs" }, { label: tTools("kinematics-calculator.title") }]} />
      <DocsHero title={tTools("kinematics-calculator.title")} version={t("version")} description={t("description")} />
