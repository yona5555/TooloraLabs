...
export const RELATED_TOOLS = ["age-calculator", "countdown-to-event-calculator", "due-date-calculator"];

export async function getDateCalculatorTocItems(): Promise<TocItem[]> {
  const t = await getTranslations("docs.tools.date-calculator");
  return [
...
export default async function DateCalculatorDocsPage() {
  const t = await getTranslations("docs.tools.date-calculator");
  ...
  return (
    <>
      <DocsBreadcrumb items={[{ label: tNav("overview"), href: "/docs" }, { label: tNav("toolsGuide"), href: "/docs" }, { label: tTools("date-calculator.title") }]} />
      <DocsHero title={tTools("date-calculator.title")} version={t("version")} description={t("description")} />
      ...
      <DocsSection id="anatomy" title={t("sectionAnatomy")}>
        ...
        <BreakdownBarDiagram
          segments={[
            { label: t("anatomy.labelWeeks"), value: 98, colorClass: "fill-blue-600 dark:fill-blue-400" },
            { label: t("anatomy.labelRemainder"), value: 2, colorClass: "fill-emerald-500 dark:fill-emerald-400" },
          ]}
