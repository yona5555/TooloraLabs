...
export const RELATED_TOOLS = ["break-even-calculator", "discount-calculator", "sales-tax-calculator", "invoice-generator", "unit-converter"];

export async function getFuelCostCalculatorTocItems(): Promise<TocItem[]> {
  const t = await getTranslations("docs.tools.fuel-cost-calculator");
  return [
...
export default async function FuelCostCalculatorDocsPage() {
  const t = await getTranslations("docs.tools.fuel-cost-calculator");
  ...
  return (
    <>
      <DocsBreadcrumb items={[{ label: tNav("overview"), href: "/docs" }, { label: tNav("toolsGuide"), href: "/docs" }, { label: tTools("fuel-cost-calculator.title") }]} />
      <DocsHero title={tTools("fuel-cost-calculator.title")} version={t("version")} description={t("description")} />
      ...
      <DocsSection id="anatomy" title={t("sectionAnatomy")}>
        ...
        <ValueComparisonBarDiagram
          bars={[
            { label: t("anatomy.labelConsumption"), value: 24, colorClass: "fill-blue-600 dark:fill-blue-400" },
            { label: t("anatomy.labelEfficiency"), value: 24, colorClass: "fill-emerald-500 dark:fill-emerald-400" },
          ]}
