...
import DeviationBarDiagram from "./DeviationBarDiagram";
import ValueComparisonBarDiagram from "@/components/docs/ValueComparisonBarDiagram";

export const RELATED_TOOLS = ["statistics-calculator", "mean-median-mode-range-calculator", "probability-calculator", "percentage-calculator", "random-number-generator"];

export async function getStandardDeviationCalculatorTocItems(): Promise<TocItem[]> {
  const t = await getTranslations("docs.tools.standard-deviation-calculator");
  return [
    { id: "formula", label: t("sectionFormula") },
    { id: "flowchart", label: t("sectionFlowchart") },
    { id: "deviations", label: t("sectionDeviations") },
...
const SAMPLE_ROWS = [
  { label: "2", value: 2, deviation: -4 },
  { label: "4", value: 4, deviation: -2 },
  { label: "6", value: 6, deviation: 0 },
  { label: "8", value: 8, deviation: 2 },
  { label: "10", value: 10, deviation: 4 },
];

export default async function StandardDeviationCalculatorDocsPage() {
  const t = await getTranslations("docs.tools.standard-deviation-calculator");
  ...
  return (
    <>
      <DocsBreadcrumb items={[{ label: tNav("overview"), href: "/docs" }, { label: tNav("toolsGuide"), href: "/docs" }, { label: tTools("standard-deviation-calculator.title") }]} />
      <DocsHero title={tTools("standard-deviation-calculator.title")} version={t("version")} description={t("description")} />
      ...
      <DocsSection id="deviations" title={t("sectionDeviations")}>
        <p className="mb-4 text-zinc-600 dark:text-zinc-300">{t("deviations.intro")}</p>
        <DeviationBarDiagram rows={SAMPLE_ROWS} meanLabel={t("deviations.meanLabel")} caption={t("deviations.caption")} />
      </DocsSection>
      ...
      <DocsSection id="anatomy" title={t("sectionAnatomy")}>
        ...
        <ValueComparisonBarDiagram
          bars={[
            { label: t("anatomy.labelPopulation"), value: 2.83, colorClass: "fill-blue-600 dark:fill-blue-400" },
            { label: t("anatomy.labelSample"), value: 3.16, colorClass: "fill-purple-500 dark:fill-purple-400" },
          ]}
