...
export const RELATED_TOOLS = ["json-formatter", "password-generator", "qr-code-generator"];

export async function getBase64ToolTocItems(): Promise<TocItem[]> {
  const t = await getTranslations("docs.tools.base64-tool");
  return [
...
export default async function Base64ToolDocsPage() {
  const t = await getTranslations("docs.tools.base64-tool");
  ...
  return (
    <>
      <DocsBreadcrumb items={[{ label: tNav("overview"), href: "/docs" }, { label: tNav("toolsGuide"), href: "/docs" }, { label: tTools("base64-tool.title") }]} />
      <DocsHero title={tTools("base64-tool.title")} version={t("version")} description={t("description")} />
