...
export const RELATED_TOOLS = ["time-calculator", "date-calculator", "countdown-to-event-calculator"];

export async function getWorldTimeConverterTocItems(): Promise<TocItem[]> {
  const t = await getTranslations("docs.tools.world-time-converter");
  return [
...
export default async function WorldTimeConverterDocsPage() {
  const t = await getTranslations("docs.tools.world-time-converter");
  ...
  return (
    <>
      <DocsBreadcrumb items={[{ label: tNav("overview"), href: "/docs" }, { label: tNav("toolsGuide"), href: "/docs" }, { label: tTools("world-time-converter.title") }]} />
      <DocsHero title={tTools("world-time-converter.title")} version={t("version")} description={t("description")} />
      ...
      <DocsSection id="anatomy" title={t("sectionAnatomy")}>
        ...
        <NumberLineMagnitudeDiagram
          markers={[
            { label: t("anatomy.labelFrom"), exponent: -5 },
            { label: t("anatomy.labelTo"), exponent: 9 },
          ]}
          highlightExponent={9}
