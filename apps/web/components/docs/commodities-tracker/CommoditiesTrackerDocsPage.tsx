...
export const RELATED_TOOLS = ["forex-converter", "crypto-converter", "unit-converter"];

export async function getCommoditiesTrackerTocItems(): Promise<TocItem[]> {
  const t = await getTranslations("docs.tools.commodities-tracker");
  return [
...
export default async function CommoditiesTrackerDocsPage() {
  const t = await getTranslations("docs.tools.commodities-tracker");
  ...
  return (
    <>
      <DocsBreadcrumb items={[{ label: tNav("overview"), href: "/docs" }, { label: tNav("toolsGuide"), href: "/docs" }, { label: tTools("commodities-tracker.title") }]} />
      <DocsHero title={tTools("commodities-tracker.title")} version={t("version")} description={t("description")} />
      ...
      <DocsSection id="anatomy" title={t("sectionAnatomy")}>
        ...
        <ValueComparisonBarDiagram
          bars={[
            { label: t("anatomy.labelGold"), value: 141, colorClass: "fill-amber-400 dark:fill-amber-500" },
            { label: t("anatomy.labelSilver"), value: 2, colorClass: "fill-zinc-400 dark:fill-zinc-500" },
            { label: t("anatomy.labelOil"), value: 79, colorClass: "fill-blue-600 dark:fill-blue-400" },
          ]}
