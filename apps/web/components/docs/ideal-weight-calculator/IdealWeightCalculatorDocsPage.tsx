...
export const RELATED_TOOLS = ["bmi-calculator", "body-fat-calculator", "bmr-calculator", "tdee-calculator", "macro-calculator"];

export async function getIdealWeightCalculatorTocItems(): Promise<TocItem[]> {
  const t = await getTranslations("docs.tools.ideal-weight-calculator");
  return [
...
export default async function IdealWeightCalculatorDocsPage() {
  const t = await getTranslations("docs.tools.ideal-weight-calculator");
  ...
  return (
    <>
      <DocsBreadcrumb items={[{ label: tNav("overview"), href: "/docs" }, { label: tNav("toolsGuide"), href: "/docs" }, { label: tTools("ideal-weight-calculator.title") }]} />
      <DocsHero title={tTools("ideal-weight-calculator.title")} version={t("version")} description={t("description")} />
      ...
      <DocsSection id="anatomy" title={t("sectionAnatomy")}>
        ...
        <ValueComparisonBarDiagram
          bars={[
            { label: t("anatomy.labelDevine"), value: 75.0, colorClass: "fill-blue-600 dark:fill-blue-400" },
            { label: t("anatomy.labelRobinson"), value: 72.6, colorClass: "fill-emerald-500 dark:fill-emerald-400" },
            { label: t("anatomy.labelMiller"), value: 71.5, colorClass: "fill-amber-500 dark:fill-amber-400" },
            { label: t("anatomy.labelHamwi"), value: 77.3, colorClass: "fill-purple-500 dark:fill-purple-400" },
          ]}
