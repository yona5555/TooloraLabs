...
export const RELATED_TOOLS = ["invoice-generator", "inventory-valuation-calculator", "discount-calculator", "sales-tax-calculator", "batch-invoice-calculator"];

export async function getBreakEvenCalculatorTocItems(): Promise<TocItem[]> {
  const t = await getTranslations("docs.tools.break-even-calculator");
  return [
...
export default async function BreakEvenCalculatorDocsPage() {
  const t = await getTranslations("docs.tools.break-even-calculator");
  ...
  return (
    <>
      <DocsBreadcrumb items={[{ label: tNav("overview"), href: "/docs" }, { label: tNav("toolsGuide"), href: "/docs" }, { label: tTools("break-even-calculator.title") }]} />
      <DocsHero title={tTools("break-even-calculator.title")} version={t("version")} description={t("description")} />
      ...
      <DocsSection id="anatomy" title={t("sectionAnatomy")}>
        ...
        <BreakEvenChartDiagram
          fixedCosts={900}
          variableCostPerUnit={20}
          pricePerUnit={50}
          breakEvenUnits={30}
          maxUnits={50}
          labelRevenue={t("anatomy.labelRevenue")}
          labelCost={t("anatomy.labelCost")}
          labelBreakEven={t("anatomy.labelBreakEven")}
