...
export const RELATED_TOOLS = ["duplicate-line-remover", "csv-json-converter", "zip-compressor"];

export async function getFileNameSanitizerTocItems(): Promise<TocItem[]> {
  const t = await getTranslations("docs.tools.file-name-sanitizer");
  return [
...
export default async function FileNameSanitizerDocsPage() {
  const t = await getTranslations("docs.tools.file-name-sanitizer");
  ...
  return (
    <>
      <DocsBreadcrumb items={[{ label: tNav("overview"), href: "/docs" }, { label: tNav("toolsGuide"), href: "/docs" }, { label: tTools("file-name-sanitizer.title") }]} />
      <DocsHero title={tTools("file-name-sanitizer.title")} version={t("version")} description={t("description")} />
      ...
      <DocsSection id="anatomy" title={t("sectionAnatomy")}>
        ...
        <ValueComparisonBarDiagram
          bars={[
            { label: t("anatomy.labelBefore"), value: 16, colorClass: "fill-orange-500 dark:fill-orange-400" },
            { label: t("anatomy.labelAfter"), value: 14, colorClass: "fill-emerald-500 dark:fill-emerald-400" },
          ]}
