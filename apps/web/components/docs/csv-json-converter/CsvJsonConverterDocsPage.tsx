...
export const RELATED_TOOLS = ["json-formatter", "file-size-converter", "duplicate-line-remover"];

export async function getCsvJsonConverterTocItems(): Promise<TocItem[]> {
  const t = await getTranslations("docs.tools.csv-json-converter");
  return [
...
export default async function CsvJsonConverterDocsPage() {
  const t = await getTranslations("docs.tools.csv-json-converter");
  ...
  return (
    <>
      <DocsBreadcrumb items={[{ label: tNav("overview"), href: "/docs" }, { label: tNav("toolsGuide"), href: "/docs" }, { label: tTools("csv-json-converter.title") }]} />
      <DocsHero title={tTools("csv-json-converter.title")} version={t("version")} description={t("description")} />
      ...
      <DocsSection id="anatomy" title={t("sectionAnatomy")}>
        ...
        <ValueComparisonBarDiagram
          bars={[
            { label: t("anatomy.labelComma"), value: 2, colorClass: "fill-blue-600 dark:fill-blue-400" },
            { label: t("anatomy.labelSemicolon"), value: 1, colorClass: "fill-emerald-500 dark:fill-emerald-400" },
            { label: t("anatomy.labelTab"), value: 0, colorClass: "fill-purple-500 dark:fill-purple-400" },
            { label: t("anatomy.labelPipe"), value: 0, colorClass: "fill-orange-500 dark:fill-orange-400" },
          ]}
