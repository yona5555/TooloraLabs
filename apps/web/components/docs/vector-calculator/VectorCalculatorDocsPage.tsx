...
import VectorArrowDiagram from "./VectorArrowDiagram";

export const RELATED_TOOLS = ["matrix-calculator", "scientific-calculator", "step-by-step-math-solver", "graphing-calculator", "percentage-calculator"];

export async function getVectorCalculatorTocItems(): Promise<TocItem[]> {
  const t = await getTranslations("docs.tools.vector-calculator");
  return [
...
export default async function VectorCalculatorDocsPage() {
  const t = await getTranslations("docs.tools.vector-calculator");
  ...
  return (
    <>
      <DocsBreadcrumb items={[{ label: tNav("overview"), href: "/docs" }, { label: tNav("toolsGuide"), href: "/docs" }, { label: tTools("vector-calculator.title") }]} />
      <DocsHero title={tTools("vector-calculator.title")} version={t("version")} description={t("description")} />
      ...
      <DocsSection id="anatomy" title={t("sectionAnatomy")}>
        ...
        <VectorArrowDiagram
          ...
          bx={4}
          by={0}
          labelA={t("anatomy.labelA")}
          labelB={t("anatomy.labelB")}
          labelSum={t("anatomy.labelSum")}
