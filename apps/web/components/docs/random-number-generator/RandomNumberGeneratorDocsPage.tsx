...
import UniformDistributionDiagram from "./UniformDistributionDiagram";
import DotPlotDiagram from "@/components/docs/DotPlotDiagram";

export const RELATED_TOOLS = ["probability-calculator", "statistics-calculator", "mean-median-mode-range-calculator", "standard-deviation-calculator", "gcf-lcm-calculator"];

export async function getRandomNumberGeneratorTocItems(): Promise<TocItem[]> {
  const t = await getTranslations("docs.tools.random-number-generator");
  return [
...
export default async function RandomNumberGeneratorDocsPage() {
  const t = await getTranslations("docs.tools.random-number-generator");
  ...
  return (
    <>
      <DocsBreadcrumb items={[{ label: tNav("overview"), href: "/docs" }, { label: tNav("toolsGuide"), href: "/docs" }, { label: tTools("random-number-generator.title") }]} />
      <DocsHero title={tTools("random-number-generator.title")} version={t("version")} description={t("description")} />
      ...
      <DocsSection id="distribution" title={t("sectionDistribution")}>
        ...
        <UniformDistributionDiagram min={1} max={100} bandLabel={t("distribution.bandLabel")} caption={t("distribution.caption")} />
      </DocsSection>

      <DocsSection id="anatomy" title={t("sectionAnatomy")}>
        <p className="mb-4 text-zinc-600 dark:text-zinc-300">{t("anatomy.intro")}</p>
        <DotPlotDiagram
          values={[7, 23, 41, 58, 62, 79, 88, 95]}
          markers={[{ value: 100, label: t("anatomy.labelMax"), colorClass: "stroke-purple-500 dark:stroke-purple-400 fill-purple-500 dark:fill-purple-400" }]}
