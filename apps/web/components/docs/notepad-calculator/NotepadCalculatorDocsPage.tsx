...
export const RELATED_TOOLS = ["scientific-calculator", "step-by-step-math-solver", "graphing-calculator", "percentage-calculator", "fraction-calculator"];

export async function getNotepadCalculatorTocItems(): Promise<TocItem[]> {
  const t = await getTranslations("docs.tools.notepad-calculator");
  return [
    { id: "formula", label: t("sectionFormula") },
    { id: "flowchart", label: t("sectionFlowchart") },
    { id: "worked-example", label: t("sectionWorkedExample") },
...
export default async function NotepadCalculatorDocsPage() {
  const t = await getTranslations("docs.tools.notepad-calculator");
  ...
  const workedExampleSteps = t.raw("workedExample.steps") as string[];

  return (
    <>
      <DocsBreadcrumb items={[{ label: tNav("overview"), href: "/docs" }, { label: tNav("toolsGuide"), href: "/docs" }, { label: tTools("notepad-calculator.title") }]} />
      <DocsHero title={tTools("notepad-calculator.title")} version={t("version")} description={t("description")} />
      ...
      <DocsSection id="worked-example" title={t("sectionWorkedExample")}>
        <p className="mb-4 text-zinc-600 dark:text-zinc-300">{t("workedExample.intro")}</p>
        <CalculationFlowchart steps={workedExampleSteps} caption={t("workedExample.caption")} />
      </DocsSection>
      ...
      <DocsSection id="anatomy" title={t("sectionAnatomy")}>
        ...
        <ValueComparisonBarDiagram
          bars={[
            { label: t("anatomy.labelBefore"), value: 100, colorClass: "fill-blue-600 dark:fill-blue-400" },
            { label: t("anatomy.labelAfter"), value: 108.5, colorClass: "fill-emerald-500 dark:fill-emerald-400" },
          ]}
