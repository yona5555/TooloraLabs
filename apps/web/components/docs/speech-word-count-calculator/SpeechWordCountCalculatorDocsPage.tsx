...
export const RELATED_TOOLS = ["reading-time-calculator", "word-counter", "readability-score-calculator"];

export async function getSpeechWordCountCalculatorTocItems(): Promise<TocItem[]> {
  const t = await getTranslations("docs.tools.speech-word-count-calculator");
  return [
...
export default async function SpeechWordCountCalculatorDocsPage() {
  const t = await getTranslations("docs.tools.speech-word-count-calculator");
  ...
  return (
    <>
      <DocsBreadcrumb items={[{ label: tNav("overview"), href: "/docs" }, { label: tNav("toolsGuide"), href: "/docs" }, { label: tTools("speech-word-count-calculator.title") }]} />
      <DocsHero title={tTools("speech-word-count-calculator.title")} version={t("version")} description={t("description")} />
      ...
      <DocsSection id="anatomy" title={t("sectionAnatomy")}>
        ...
        <ValueComparisonBarDiagram
          bars={[
            { label: t("anatomy.labelFormal"), value: 229, colorClass: "fill-blue-600 dark:fill-blue-400" },
            { label: t("anatomy.labelNormal"), value: 180, colorClass: "fill-emerald-500 dark:fill-emerald-400" },
            { label: t("anatomy.labelFast"), value: 148, colorClass: "fill-purple-500 dark:fill-purple-400" },
          ]}
